...
| **N10** | **There is no client-initiated open, and the absence was never a decision.** `ui/openWindow` is a notify the SERVER sends and the client only has an `onNotify` for it; `ClientWindows` has no `open`; the sole client-to-server window message is `ui/close`, whose own comment calls it *"the direction that did not exist"*. Server-push is correct as a **default** and matches Minecraft's own pipeline (`openMenu` is server-side; a client-opened container desyncs inventory). What is missing is that nobody wrote it down — none of the three plan documents mentions the question — so it reads as an oversight rather than as the authority decision it should be. **It is already possible by hand**, and `mc/example/MachineExample` is the idiom: the client sends a plain `machine/open` notify through `Protocols` and the server calls `ServerWindows.open`. So this is not a plumbing gap; a round trip works today. It is a **policy** gap, and the policy is the hard half — *what* may a client ask to open, and how does a server refuse without the client waiting on nothing? Two sub-questions fall out of any answer: a refused request needs a **refusal the client can act on** rather than silence (the same complaint the P-series makes about a dropped request), and a client asking twice must not open two windows, which `key()` already settles for the server-initiated case. **Recommended shape:** keep push as the default and add an opt-in — a server declares which `UiType`s are client-openable and with what argument, so the authority is stated once rather than re-derived per mod. Belongs with N4 (the close veto), the other place the client got a voice | **C** | ICCCM: a client maps its own window and the WM may refuse. The web has no equivalent — a page cannot open a server's UI at all. Minecraft's `openGui` goes through a GuiHandler the mod registers, which is exactly the "declare what is openable" shape |
