...
// AND THE OTHER TWO, for a loader that bundles them -- `-PcgBundleBands` on 1.7.10, band 17 on 1.20.x. A
// band a jar does not carry is fetched from where download/locations.json lists it: the GTNH case, 1.7.10
// on Java 17 selecting band 17. The root build reads all three bundles too, to hold that file to them.
...
tasks.named("check") { dependsOn("checkEngineBands", "smokeEngineBands", ":checkDownloadLocations") }
