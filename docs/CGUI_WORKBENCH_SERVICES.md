...
| Separators are **elements**, not borders | A separator has its own colour and its own inset, and a border has neither: it takes the box's border colour and runs the whole edge. Each side does draw at its own width, so `border-left-width` alone is a rule rather than a rectangle -- that is not the reason. `Breadcrumbs` spells its separators the same way |
