...
| `UITreeTraversal` | 308 | — |
| `EventListenerGroup`, `ElementRegistry`, `UIFrameTicker`, `Ui`, `Transform`, `input/*`, `keymap/*` | 1,633 | — |
| **Total** | **11,405** | |
...
| `Transform` — ordered op list, `IDENTITY` (empty list — snaps against anything), `applyTo`, no `matrix()` | unchanged | the row *"`Transform.IDENTITY` … SNAPS at the halfway point"* survives untouched |
