 ...
 * local disk, a {@code ServerUiSession}, a server binding, and an {@code InMemoryTransport} pair
 ...
 * {@link Workspace} over the connection {@link CgUiConnections} already opened. The transport, the
...
public final class Mc1710Workspace {
     ...
     * with every callback anything registered on it. {@link Workspace#rebind} swaps the wire and
    ...
    Workspace client() {
        ...
        if (workspace == null) {
            ...
            bound = connection;
        } else if (bound != connection) {
            // THE WIRE MOVED. Rebound rather than rebuilt, so a window retained across the disconnect
            // keeps the object it is holding and every callback registered on it.
            workspace.rebind(connection);
            bound = connection;
        }
        return workspace;
    }

    /** Which wire the workspace is currently bound to, so a rebind can be told from a no-op. */
