 ...
 * }</pre>
 *
 * <p>It lives in this jar because every one of its probes names {@code language/} — a script through
 * the Run command, live bytes out of the {@code ScriptService}, the member list a completion provider
 * answers. The host's own autotest opens the editor and photographs it, which needs none of them, so
 * an install without this jar still runs the whole capture.</p>
 */
public final class LanguageAutoTest1710 {
    ...
    /**
     * Puts every probe on the frame it belongs to. Off unless {@code -Dcrystalgui.autotest=true}, so
     * the language jar costs the host nothing on an ordinary run.
     */
    public static void register() {
        if (!CgUiAutoTest.ENABLED) return;
        // BEFORE the capture frame, so a probe still leaves a photograph behind.
        if (RUN_SCRIPT_ON_FRAME > 0) {
            CgUiAutoTest.onFrame(RUN_SCRIPT_ON_FRAME, LanguageAutoTest1710::runScriptOnce);
        }
        CgUiAutoTest.onFrame(5, LanguageAutoTest1710::probeLiveBytesOnce);
        CgUiAutoTest.onFrame(6, LanguageAutoTest1710::probeCompletionOnce);
        // ...and asked much later, because the analysis behind each one is debounced onto a worker that
        // drains on THIS thread.
        CgUiAutoTest.onFrame(60, LanguageAutoTest1710::reportCompletionProbes);
    }
    ...
    /** Which painted frame runs it — before {@link CgUiAutoTest#CAPTURE_ON_FRAME}, so a capture still happens. */
    ...
    static void runScriptOnce() {
        if (!CgUiAutoTest.ENABLED || SCRIPT == null || scriptRun) return;
        scriptRun = true;
     ...
     * them differently. The live side is {@link LaunchWrapperBytes} — exactly
    ...
    static void probeLiveBytesOnce() {
        if (!CgUiAutoTest.ENABLED || BYTES_PROBE == null || bytesProbed) return;
        bytesProbed = true;
        try {
    ...
    static void probeCompletionOnce() {
        if (!CgUiAutoTest.ENABLED || !COMPLETE_PROBE || completionProbed) return;
    ...
    static void reportCompletionProbes() {
        if (!CgUiAutoTest.ENABLED || !COMPLETE_PROBE || PENDING.isEmpty()) return;
