...
final class ServiceFixtures {
    ...
    static Node at(String id, float x, float y, float width, float height) {
    ...
    static void frame(Document document) {
        document.frame(0.016f, W, H);
    }

    static void move(Document document, float x, float y) {
        document.input().consumeMouseEvent(
                new CgSystemInput.Mouse.Event((int) x, (int) y, 0, 0, -1, false, 0f, now()));
    }

    static void press(Document document, float x, float y) {
        press(document, x, y, 0);
    }

    static void press(Document document, float x, float y, int button) {
        move(document, x, y);
        document.input().consumeMouseEvent(
                new CgSystemInput.Mouse.Event((int) x, (int) y, 0, 0, button, true, 0f, now()));
    }

    static void release(Document document, float x, float y) {
        release(document, x, y, 0);
    }

    static void release(Document document, float x, float y, int button) {
        move(document, x, y);
        document.input().consumeMouseEvent(
                new CgSystemInput.Mouse.Event((int) x, (int) y, 0, 0, button, false, 0f, now()));
    }

    static void wheel(Document document, float notches) {
    ...
    static boolean key(Document document, int keyCode, boolean pressed) {
    ...
    static <T extends UIEvent> void onTarget(Node node, Class<T> type, UIEvent.Listener<Node, T> listener) {
    ...
    static <T extends UIEvent> void on(Node node, Class<T> type, UIEvent.Listener<Node, T> listener) {
