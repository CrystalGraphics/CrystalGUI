 ...
 * {@link Animation.Hook}, on the UI thread, like any other widget reading its model.</p>
 *
...
public class ProgressStatusItem extends UIElement {
     ...
     *
     * <p>{@code registerTicker} lives on {@link UIDocument}, so a ticker can only be registered once there
    ...
    /**
     * Private, so the item's own public surface has no {@code tickFrame} to call by mistake.
     *
     * <p>A method reference rather than the inner class it was: a per-frame hook is a functional
     * interface here, so the class existed only to keep the method off the public surface — which
    ...
    private void refresh() {
        ...
        if (active.size() != shownCount) {
            ...
            StyleGroup.inlinePipeline(count.getStyle().getLayoutGroup(),
