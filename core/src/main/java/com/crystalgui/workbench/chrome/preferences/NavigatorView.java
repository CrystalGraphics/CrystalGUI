...
public class NavigatorView<T> extends UIElement {
    ...
    public NavigatorView() {
        ...
        split.first().append(sidebar);
        split.second().append(detail);
     ...
     *
     * <p>The old one deferred to a TICKER, on the reasoning that a style write inside the layout pass
     * is the one thing that hook must not do. The reason is gone with the pass it was about: layout
     * here is one pass with no feedback into it, and {@code afterLayout} runs once it has finished —
     * so the write lands on the next frame either way, with no second mechanism to arrange it.</p>
     */
    @Override
    ...
    public void giveFocus() {
        ...
        if (window != null && search != null) window.focus().requestFocus(search.input());
    }
