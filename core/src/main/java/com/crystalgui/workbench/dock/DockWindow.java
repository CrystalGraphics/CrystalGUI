 ...
 * {@code UIDocument}, one {@code UIDragController} and one hit-test, so a drag out of this window and back
...
public class DockWindow extends WindowFrame {
     ...
     *
     * <p>The old engine had this class answer {@code "window"} from {@code tagName()}, which the new
     * engine cannot do: a {@code Name} is registered to a FACTORY, so two classes claiming one name
     * makes a description ambiguous to decode. Inheriting {@code WindowFrame.NAME} is the same thing
     * wearing a nicer hat and {@code NodeKindsCoverageTest} fails it. So the sheet is keyed on a
     * class every frame carries, and this declares what it actually is.</p>
     */
    public static final Name NAME = Name.of("dockwindow");
     ...
     */
    // NO `NAME` OF ITS OWN, DELIBERATELY -- it inherits {@link WindowFrame#NAME}, so a sheet's
    // `window` rules all reach it and {@link #DOCK_WINDOW_CLASS} carries whatever differs. That is
    // the "everything the supertype has, plus a modifier class" case; a dock window without a
    // window's appearance reads as a widget that was never built rather than one left unstyled.
    ...
    private void closeIfEmptyOnceIdle() {
        ...
        window.animation().every(this, delta -> {
            ...
            if (window.input().mode(Drag.class) != null) return true;
            closePending = false;
            closeIfEmpty();
            return false;
        });
