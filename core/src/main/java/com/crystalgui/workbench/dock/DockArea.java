...
public class DockArea extends UIElement {
    ...
    /**
     * Registers the per-frame hook when this area joins a document.
     *
     * <p><b>An attach hook where the old engine rode on the first layout.</b> There was no attach
     * hook on the old element, so registration went in {@code onLayoutChanged} with an idempotence
     * guard behind it — and the hook it registered was one-way, so a hidden area went on ticking
     * until the hook itself returned false. Both problems are gone: {@code connected()} fires at the
     * one right moment, and {@link com.crystalgui.ui.service.Animation#every} is OWNED, so the hook
     * stops when the node leaves the tree rather than by remembering to stop itself.</p>
     */
    @Override
     ...
     *
     * <p><b>Cheap</b>, because {@link #rebuild()} opens with {@code content.removeAll()}: the whole
     ...
     * {@code UIDocument.registerElement x181} and a 21ms frame.</p>
     *
    ...
    private void closePanelDiscardingImpl(DockPanelRef panel) {
        ...
        // the construction. Measured on a close with two tabs open: `UIDocument.registerElement:890 x181`,
    ...
    void installTabDrag(DockGroup group, DockPanelRef panel, Tab tab) {
        tab.events.getGroup(MouseEvent.Down.class).attachListener((el, event) -> {
            ...
            // The PAYLOAD overload, which is the seven-argument one -- there is no
            // (source, x, y, payload, listener) form, because a drag that carries something also has
            // a button and a threshold worth stating rather than inheriting silently.
            Drag.start(tab,
                    event.getPosition().x(), event.getPosition().y(),
                    CgMouseCodes.LEFT_BUTTON,
                    DockDragPayload.ofPanel(this, group.leaf(), panel),
                    Drag.DEFAULT_THRESHOLD_PX,
                    new Drag.Listener() {
                        @Override
                        ...
                        public void onDragEnd(float mx, float my) {
                            ...
                            boolean accepted = live != null && live.isDropAccepted();
                            group.endTabDrag();
                            endDragVisuals();
                            // ── THE TEAR-OUT (W9) ───────────────────────────────────────────────
                            ...
                            if (containsSurfacePoint(pointer.x(), pointer.y())) return;
    ...
    private void parkGhost() {
        if (dragGhost.parent() != null) return;
    ...
    private boolean updatePreview(DockDragPayload payload, float pointerX, float pointerY) {
        Box areaBox = box();
        if (areaBox == null) return false;
        // Already relative to this area's own origin -- see the note in DockGroup.stripBandContains.
        var localArea = toLocal(pointerX, pointerY);
        float areaX = localArea.x();
        float areaY = localArea.y();
        ...
        DockDropZone outer = DockDropZones.forOuterEdge(areaX, areaY, areaBox.width(), areaBox.height());
        if (outer != null) {
    ...
    private DockGroup groupUnder(float screenX, float screenY) {
        for (DockGroup group : groups.values()) {
            if (group.document() != null && group.containsSurfacePoint(screenX, screenY)) return group;
        }
        return null;
    }
