...
/**
 * Parses CSS's {@code transform} function list into a {@link Transform}.
 *
 * <pre>
 ...
 * {@code scale(2) translate(10px)}. See {@link Transform} for why the value type is a list.</p>
 *
 ...
 */
public class TransformValue extends StyleValue<Transform> {
    ...
    @Override
    protected @Nullable Transform doCompute(String rawValue) {
        return parse(rawValue);
    }

    static @Nullable Transform parse(String rawValue) {
        if (rawValue == null) return null;
        String trimmed = rawValue.trim();
        if (trimmed.isEmpty()) return null;
        if (trimmed.equalsIgnoreCase("none")) return Transform.IDENTITY;

        List<Transform.Op> ops = new ArrayList<>();
        for (String token : CssParsingUtil.splitFunctionList(trimmed)) {
            Transform.Op op = parseFunction(token);
            if (op == null) return null;
            ops.add(op);
        }
        return ops.isEmpty() ? null : Transform.of(ops);
    }

    /** @return the op, or {@code null} if {@code token} is not a supported {@code name(args)} call. */
    private static @Nullable Transform.Op parseFunction(String token) {
        ...
        return switch (name) {
            ...
            // matrix() is deliberately unsupported — see Transform's "known divergences".
            default -> null;
        };
    }

    private static @Nullable Transform.Op translate(String x, String y) {
        LengthPercent lx = LengthPercent.parse(x);
        LengthPercent ly = LengthPercent.parse(y);
        return (lx == null || ly == null) ? null : Transform.Op.translate(lx, ly);
    }

    private static @Nullable Transform.Op scale(String x, String y) {
        Float fx = number(x);
        Float fy = number(y);
        return (fx == null || fy == null) ? null : Transform.Op.scale(fx, fy);
    }

    private static @Nullable Transform.Op rotate(String angle) {
        Float radians = CssAngle.parse(angle);
        return radians == null ? null : Transform.Op.rotate(radians);
    }

    private static @Nullable Transform.Op skew(String x, String y) {
        Float rx = CssAngle.parse(x);
        Float ry = CssAngle.parse(y);
        return (rx == null || ry == null) ? null : Transform.Op.skew(rx, ry);
    }
