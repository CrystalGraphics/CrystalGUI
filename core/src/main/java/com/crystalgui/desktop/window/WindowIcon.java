...
public class WindowIcon extends UINode {
    ...
    /**
     * On the element. Sizes come from the context class beside it ({@code __icon__},
     * {@code __pre-icon__}).
     *
     * <p>A CLASS, not the tag and not a part: the sizing rules are written against the CONTEXT the icon
     * sits in rather than against the icon, so a caller styling a slot needs a name it can put on the
     * slot's own child. That is also why it stays after the port — the tag would work for the widget
     * and not for the plain box a caller borrows the vocabulary with.</p>
     */
    public static final String ICON_CLASS = "__window-icon__";
    /** The letter drawn when there is no icon. {@code .__window-icon__::part(monogram)} in a sheet. */
    public static final String MONOGRAM_PART = "monogram";
    ...
    public WindowIcon() {
        ...
        monogram.set(Attribute.PART, MONOGRAM_PART);
        monogram.setHitTest(false);
        monogram.setDisplayed(false);
        shadow.append(monogram);
    ...
    public String monogram() {
        return showingMonogram ? monogram.text() : "";
    }
}
