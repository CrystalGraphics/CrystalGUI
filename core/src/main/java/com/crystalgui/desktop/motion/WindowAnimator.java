...
public final class WindowAnimator {
    ...
    public WindowAnimator(WindowFrame frame) {
        this.frame = frame;
    }
     ...
     *             invisible.
     */
    public void playClose(Runnable then) {
        if (!enabled) {
            then.run();
            return;
        }
     ...
     */
    public void playMinimize(Runnable then) {
        if (!enabled) {
            then.run();
            return;
        }
    ...
    private UITransform towardTaskbar() {
        ...
        if (area == null || area.width() <= 0f || area.height() <= 0f) return null;
     ...
     *
     * <p><b>The target is CONVERTED into the frame's own space, and this used to say they were
     * already comparable.</b> They were: the old accessor accumulated through every ancestor, so a
     * taskbar entry's origin and a window's origin were both absolute. {@code Box.x()} is the offset
     ...
     */
    public boolean playShrink(float fromWidth, float fromHeight, float toWidth, float toHeight, Runnable settle) {
        ...
        // instead -- and a dropped hook does NOT run its completion, which is the same outcome
        // `cancel()` is documented to want ("a cancelled minimise must not go on to hide the window
    ...
    public boolean playResize(float fromLeft, float fromTop, float fromWidth, float fromHeight,
    ...
    /**
     * Ends whatever is running from outside — for a caller that must not wait for it.
     *
     * <p><b>A timeline's completion is unreachable by waiting</b>, because its clock advances on real
     * rendered time that never elapses across instant frames. Every ending in this class runs through
     * the same {@code cancel()}, so driving that is how a test reaches the teardown at all — which is
     * the standing rule the animation tests were written from.</p>
     */
    public void cancel() {
        cancelCurrent();
    }
