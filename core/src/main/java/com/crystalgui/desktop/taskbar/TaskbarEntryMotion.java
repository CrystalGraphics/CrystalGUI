 ...
 */
final class TaskbarEntryMotion implements WindowMotion {
    ...
    private TaskbarEntryMotion(UINode entry, boolean opening, Runnable onDone) {
        ...
        // THE BOX, not an ANIMATION-origin slot. The compositor channel is what this engine gives a
        // timeline -- above the cascade, layout-free, and withdrawn with a null rather than handed a
        // resting value that would then outrank every rule wanting to fade an entry.
        setOpacity(opening ? 0f : 1f);
        if (opening) {
    ...
    boolean start() {
        UIDocument window = entry.document();
        if (window == null) return false;
        window.animation().every(entry, this::frame);
        return true;
    }
    ...
    /** Writes the entry's opacity on the compositor channel; {@code null} hands it back to the sheet. */
    private void setOpacity(@Nullable Float opacity) {
        Box box = entry.box();
        if (box != null) box.setOpacity(opacity);
    }
