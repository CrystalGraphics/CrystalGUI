...
public class TableView<T> extends ListView<T> {
    ...
    public List<Float> resolvedWidths() {
        ...
        float leftover = Math.max(0f, getClientWidth() - fixed);
    ...
    private UIElement newDivider(TableColumn<T> column, int columnIndex) {
        ...
        divider.onMouseDown.attachListener((el, event) -> {
            ...
            drag.startDrag(el, event.getPosition().x(), event.getPosition().y(),
    ...
    private void installRowRenderer() {
        setRenderer(new ListRenderer<T>() {
            ...
            public void bind(T item, int index, UIElement template) {
                List<Float> widths = resolvedWidths();
                for (int i = 0; i < columns.size() && i < template.getChildren().size(); i++) {
                    TableColumn<T> column = columns.get(i);
                    UIElement cell = template.getChildren().get(i);
                    final float width = i < widths.size() ? widths.get(i) : column.getWidth();
                    StyleGroup.defaultPipeline(cell.getStyle().getLayoutGroup(), l -> l.width(width));
                    if (column.getCellRenderer() != null) {
                        column.getCellRenderer().bind(item, index, cell);
                    } else {
                        ((UIText) cell.getChildren().get(0)).setText(column.getValue().apply(item));
                    }
                }
            }
        });
    ...
    public float getClientHeight() {
        return Math.max(0f, super.getClientHeight() - HEADER_HEIGHT);
    }
     ...
     * {@link #getClientWidth()}, which is <b>zero</b> until the first layout — so a header built at
