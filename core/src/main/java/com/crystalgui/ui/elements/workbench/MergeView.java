 ...
 */
public final class MergeView extends UIElement implements UIFrameTicker {
    ...
    private void syncScroll(@Nullable TextEditor zoomed) {
        float mine = minePane.getScrollTop();
        float result = resultPane.getScrollTop();
        float theirs = theirsPane.getScrollTop();
        ...
        lastMine = minePane.getScrollTop();
        lastResult = resultPane.getScrollTop();
        lastTheirs = theirsPane.getScrollTop();
    }
    ...
    private void alignPane(TextEditor pane, TextEditor moved, int line, float fraction) {
        ...
        pane.setScrollImmediate(pane.getScrollLeft(), line * Math.max(1f, pane.lineHeight()) + fraction);
    }
