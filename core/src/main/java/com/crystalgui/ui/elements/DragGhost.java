 ...
 *       the stylesheet — and this is the one that costs a session. {@code UIWindow.init()} runs its first
 ...
 * field on {@link UIWindow}. Neither is worth it for a saving of one field: {@link #parkIn} is also the
...
public class DragGhost extends UIElement {
     ...
     */
    public DragGhost follow(@Nullable UIWindow window, @Nullable String iconName, String text) {
        label.setText(text == null ? "" : text);
        setIcon(iconName);
        if (window == null) return this;
        window.getInputHandler().getDragController().setGhost(this, anchor);
        return this;
    }
    ...
    /** @see #follow(UIWindow, String, String) */
    public DragGhost follow(@Nullable UIWindow window, String text) {
    ...
    private void setIcon(@Nullable String iconName) {
        ...
        StyleGroup.importantPipeline(icon.getStyle().getGeneralGroup(), g -> g.overlay(applied));
        boolean shown = applied != CgUiDrawable.EMPTY;
        StyleGroup.importantPipeline(icon.getStyle().getLayoutGroup(),
                l -> l.display(shown ? TaffyDisplay.FLEX : TaffyDisplay.NONE));
    }

    /** It builds its own two parts; nothing else belongs in it. */
    @Override
