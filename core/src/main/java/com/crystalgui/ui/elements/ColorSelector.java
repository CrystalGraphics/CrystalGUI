...
public class ColorSelector extends UIElement {
    ...
    public ColorSelector() {
        ...
        buildSwatches();
        buildWheel();
        addInternalChild(left);
        buildModeChooser();
        buildChannels();
        buildHexRow();
        addInternalChild(side);
    ...
    private void buildWheel() {
        ...
        ring.addInternalChild(square);
        ring.addInternalChild(ringHandle);
        square.addInternalChild(squareHandle);
        wheel.addInternalChild(ring);
        left.addInternalChild(wheel);
        ...
        dragSurface(ring, (x, y) -> {
            ...
            hue = hueFromOffset(x - ring.getRuntimeCache().getWidth() * 0.5f,
                    y - ring.getRuntimeCache().getHeight() * 0.5f);
            applyHsv();
        });
        dragSurface(square, (x, y) -> {
            saturation = clamp01(x / Math.max(1f, square.getRuntimeCache().getWidth()));
            // 1 - y because value runs UP the square while coordinates run down it.
            value = 1f - clamp01(y / Math.max(1f, square.getRuntimeCache().getHeight()));
            applyHsv();
        });
    }
    ...
    private void dragSurface(UIElement surface, java.util.function.BiConsumer<Float, Float> onPoint) {
        ...
        surface.onMouseDown.attachListener((el, event) -> {
            ...
            var local = surface.screenToLocal(rawX, rawY);
     ...
     */
    private static float withinX(UIElement surface, float localX) {
        return localX - surface.getRuntimeCache().getX();
    }

    private static float withinY(UIElement surface, float localY) {
        return localY - surface.getRuntimeCache().getY();
    }
    ...
    private void buildSwatches() {
        ...
        swatches.addChild(originalSwatch);
        swatches.addChild(newSwatch);
        left.addInternalChild(swatches);
    }
    ...
    private void buildModeChooser() {
        ...
        side.addInternalChild(modeChooser);
    }
    ...
    private void buildChannels() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            rows.add(row);
            channels.addInternalChild(row.root);
        }
        side.addInternalChild(channels);
    }
    ...
    private void buildHexRow() {
        ...
        row.addChild(label);
        row.addChild(hexField);
        side.addInternalChild(row);
    }
