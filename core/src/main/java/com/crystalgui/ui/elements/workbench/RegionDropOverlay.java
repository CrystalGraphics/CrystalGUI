...
public class RegionDropOverlay extends UIElement {
    ...
    private RegionDropZones.Target resolve(UIElement host, ReadOnlyVec2f position) {
        Vector2f local = host.screenToLocal(position.x(), position.y());
        var cache = host.getRuntimeCache();
        float width = cache.getWidth();
        float height = cache.getHeight();
        // LOCAL COORDINATES ARE NOT OFFSETS. screenToLocal returns a point in the frame getX()/getY()
        // live in, so the host's own origin has to come off -- the same subtraction UIDragController's
        // grab offset needed, and the same bug if it is missed: every band would be measured from the
        // window's corner rather than the workbench's.
        float x = local.x - cache.getX();
        float y = local.y - cache.getY();
    ...
    private float leftBand(UIElement host) {
        ...
        return region.getRuntimeCache().getX() + region.getRuntimeCache().getWidth()
                - host.getRuntimeCache().getX();
    }
    ...
    private float rightBand(UIElement host) {
        ...
        if (region == null) return 0f;
        var hostCache = host.getRuntimeCache();
        return hostCache.getX() + hostCache.getWidth() - region.getRuntimeCache().getX();
    }
    ...
    private float bottomBand(UIElement host) {
        ...
        if (region == null) return 0f;
        var hostCache = host.getRuntimeCache();
        return hostCache.getY() + hostCache.getHeight() - region.getRuntimeCache().getY();
    }
    ...
    private void show(UIElement host, @Nullable RegionDropZones.Target slot) {
        ...
        StyleGroup.importantPipeline(preview.getStyle().getLayoutGroup(), l -> l
    ...
    private float[] regionRect(UIElement host, DockRegion region) {
        ...
        if (open != null) {
            var cache = open.getRuntimeCache();
            return new float[]{cache.getX() - hostCache.getX(), cache.getY() - hostCache.getY(),
                    cache.getWidth(), cache.getHeight()};
        }
        return RegionDropZones.fallbackRect(region, hostCache.getWidth(), hostCache.getHeight(),
    ...
    private float railInset() {
        for (StripeView stripe : workbench.stripes()) {
            float width = stripe.getRuntimeCache().getWidth();
