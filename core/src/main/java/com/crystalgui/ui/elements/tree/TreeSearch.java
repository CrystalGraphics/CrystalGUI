...
public final class TreeSearch<T> {
    ...
    private void build(UIElement host, int index) {
        ...
        host.insertInternalChildAt(bar, Math.max(0, Math.min(index, host.getChildren().size())));
        installTypeAhead();
        apply();
    }
    ...
    public void close() {
        ...
        if (window != null && tree.focusable()) window.getInputHandler().requestPointerFocus(tree);
    }
    ...
    private void focusInput() {
        UIWindow window = input.getAttachedWindow();
        if (window == null) return;
        window.getInputHandler().requestPointerFocus(input);
