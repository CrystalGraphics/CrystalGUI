 ...
 */
public class MenuBarView extends UIElement implements UIFrameTicker {
    ...
    private void buildBurger() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            burger.addInternalChild(stripe);
        }
    ...
    private boolean shouldAutoCollapse() {
        ...
        float available = getTaffyLayout().contentBoxWidth();
    ...
    private void applyCollapsed(boolean value) {
        ...
        StyleGroup.importantPipeline(burger.getStyle().getLayoutGroup(),
                l -> l.display(value ? TaffyDisplay.FLEX : TaffyDisplay.NONE));
        for (Title title : titles) {
            StyleGroup.importantPipeline(title.getStyle().getLayoutGroup(),
     ...
     */
    private UIElement contextSource(UIWindow window) {
    ...
    private boolean isUsableSource(@Nullable UIElement element) {
        ...
        if (element == null || element.getAttachedWindow() == null) return false;
    ...
    protected void onWindowChanged(@Nullable UIWindow previous, @Nullable UIWindow current) {
        ...
        current.ui.rootElement.events.getGroup(KeyboardEvent.Down.class)
                .attachListener((element, event) -> {
                    if (getAttachedWindow() == current) onKeyDown(event);
                }, true, false);
        ...
        current.ui.rootElement.events.getGroup(KeyboardEvent.Down.class)
                .attachListener((element, event) -> {
                    if (getAttachedWindow() != current || openTitle == null) return;
        ...
        current.ui.rootElement.events.getGroup(MouseEvent.Up.class)
                .attachListener((element, event) -> {
    ...
    private final class Title extends UIElement {
        ...
        /** Its settled width. Here rather than at the call site because {@code getTaffyLayout} is
         * protected on {@code UIElement}, and protected reaches only through one's OWN type. */
        float laidOutWidth() {
            return getTaffyLayout().size().getWidth();
        }
