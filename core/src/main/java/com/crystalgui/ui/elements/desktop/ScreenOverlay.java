...
public final class ScreenOverlay {
    ...
    public ScreenOverlay(UIWindow window) {
        this.window = window;
    }
    ...
    public boolean offerMouse(int xPx, int yPx, int button, boolean pressed, float wheel) {
        ...
        boolean captured = window.getInputHandler().hasPointerCapture();

        if (!isMove && !captured && window.overlayHitTest(xPx, yPx) == null) {
            ...
            if (pressed) {
                ...
                if (focused != null) window.getInputHandler().blurIfFocused(focused);
                ...
                if (!window.getAutoPopovers().isEmpty()) window.lightDismiss(null);
            }
            return false;
        }
