...
final class EditorFind {
     ...
     */
    int find(@Nullable SearchQuery query) {
    ...
    int find(String query, boolean caseSensitive) {
     ...
     */
    boolean toggleExcludeCurrentMatch() {
    ...
    void setPreserveCase(boolean value) {
    ...
    SearchReplaceBar bar() {
        if (bar == null) {
            ...
            editor.addInternalChild(bar);
            bar.onClosed.connect(() -> {
                UIWindow window = editor.getAttachedWindow();
                if (window != null) window.getInputHandler().requestPointerFocus(editor);
            });
        }
        return bar;
    }
