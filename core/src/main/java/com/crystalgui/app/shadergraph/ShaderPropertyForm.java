...
public final class ShaderPropertyForm {
    ...
    /**
     * Records which menu entry created a property, so a pill can say {@code Color} and not
     * {@code vec4}.
     *
     * <p>Here rather than on {@code BlackboardPanel}, which declared it until M6.4: two menu entries
     * can share a wire type, so the entry is the only thing that distinguishes them — and what a
     * property IS is a fact about the property, not about the panel that offers the menu. This class
     * is the one that knows the literal form of a default, so it is the one that has to know the
     * distinction; reading it off the widget made a model-shaped class transitively a widget.</p>
     */
    public static final String KIND_OPTION = "kind";

    /** The one menu entry whose wire type does not name it — see {@code BlackboardPanel.TYPES}. */
    ...
    public static ConfigDescriptor describeDefault(GraphProperty property) {
        if (KIND_COLOR.equals(property.option(KIND_OPTION))) {
