...
public final class ShaderGraphBridge {
    ...
    public static NodeTypeRegistry asNodeLibrary(CgShaderNodeRegistry shaderNodes) {
        registerPortTypes();
        // THE FIELD WIDGETS ARE NOT INSTALLED HERE any more -- see ShaderNodeLibrary, which is what a
        // caller wanting an editable library asks. The timing was right and the placement made this
        // otherwise widget-free map into a class that cannot compile without the UI.
    ...
    /**
     * A {@link com.crystalgui.graph.port.PortType} carrying GLSL's promotion rule.
     *
    ...
    public record GlslPortType(String id, int arity) implements
            com.crystalgui.graph.port.PortType {
        ...
        @Override
        public boolean isCompatibleWith(com.crystalgui.graph.port.PortType other) {
