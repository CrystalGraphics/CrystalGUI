...
public final class GraphCommands {
    ...
    private static void declare(CommandRegistry registry) {
        ...
        registry.register(Command.of(CREATE_NODE, "Create Node")
                ...
                .run(context -> withGraph(context, graph -> {
                    var window = graph.document();
                    if (window != null) {
                        var pointer = window.input().pointer();
                        if (graph.containsSurfacePoint(pointer.x(), pointer.y())) {
    ...
    private static void pasteInto(GraphView graph, @Nullable GraphDocument clip) {
        ...
        UIDocument window = graph.document();
        if (window != null) {
            var pointer = window.input().pointer();
            if (graph.containsSurfacePoint(pointer.x(), pointer.y())) {
                var world = graph.screenToWorld(pointer.x(), pointer.y());
     ...
     *
     * <p>Was a hand-rolled walk up {@code parent()}. It is now one line over {@link DataContext},
