...
public class Popover extends UINode {
    ...
    /**
     * This popover's shadow tree, for a subclass with parts of its own ({@code Menu}'s item column).
     *
     * <p>A bare {@code Popover} has none: its content IS the caller's, so light children are exactly
     * right and a slot would be a box in the way. The root is created on first ask, which is what
     * lets a subclass add parts without every plain popover paying for one.</p>
     */
    protected final ShadowRoot shadow() {
        if (shadowRoot() == null) {
            ShadowRoot root = attachShadow();
            // The caller's content still has to appear, so the default slot goes in FIRST -- a shadow
            // tree with no slot hides every light child, silently.
            root.append(new UISlot());
        }
        return shadowRoot();
    }
    ...
    /** For a subclass that is its own kind — {@code Menu}. See {@link Button#Button(Name, String)}. */
    protected Popover(Name name) {
        super(name);
    ...
    public Popover hide() {
        ...
        UIDocument openWindow = document();
        if (openWindow != null && mode == Mode.AUTO) openWindow.dismiss().lightDismiss(this);
        ...
        if (window != null) {
            ...
            if (focusBeforeOpen != null && focusBeforeOpen.document() == window) {
                ...
                if (focusBeforeOpenWasVisible) {
                    ...
                    window.focus().requestPointerFocus(focusBeforeOpen);
                }
            }
        }
    ...
    /**
     * The close-watcher hook — Escape, and the target of {@link com.crystalgui.ui.service.Dismiss#lightDismiss}.
     *
    ...
    public Popover moveTo(float rootLeft, float rootTop) {
        ...
        StyleGroup.inlinePipeline(getStyle().getLayoutGroup(), l -> l.left(rootLeft).top(rootTop));
        return this;
    }
    ...
    private void startPlacementTicker() {
        ...
        // AFTER LAYOUT, which is what the onLayoutChanged override this replaced was for. Placement
        // flips and clamps against this popover's OWN width and height, and an ordinary hook runs
        // before layout -- so on the opening frame it would place the popover as if it were a point
        // and correct on the next: a visible one-frame jump.
        window.animation().afterLayout(this, this::tickPlacement);
    }

    /** Keeps placement current while open, then drops itself. */
