...
public class Slider extends UINode {
    ...
    /** The filled part of the track. {@code slider::part(fill)} in a sheet. */
    public static final String FILL_PART = "fill";
    /** The grab handle. */
    public static final String THUMB_PART = "thumb";
    /** The empty remainder, which is what pushes the thumb -- see {@link #applyFraction}. */
    ...
    public Slider() {
        ...
        this.fill = newPart(FILL_PART);
        this.thumb = newPart(THUMB_PART);
        this.spacer = newPart(SPACER_PART);
    ...
    private void applyFraction() {
        float f = getFraction();
        // INLINE, not IMPORTANT: the split IS the value, so it has to outrank the sheet's resting
        // rule and INLINE does. The new engine may not write at IMPORTANT, which sits above every
        // author origin including `!important`.
        StyleGroup.inlinePipeline(fill.getStyle().getLayoutGroup(), l -> l.flexGrow(f));
        StyleGroup.inlinePipeline(spacer.getStyle().getLayoutGroup(), l -> l.flexGrow(1f - f));
    }
