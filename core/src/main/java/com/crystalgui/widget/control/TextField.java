...
public class TextField extends UIElement implements Measurable {
    ...
    public TextField() {
        ...
        this.events.getGroup(MouseEvent.Down.class).attachListener((el, event) -> {
            if (!isEnabled()) return;
            int index = indexAt(toLocal(event.getPosition().x(), event.getPosition().y()).x());
    ...
    /** Owned by this node, so leaving the tree and being frozen both drop it. */
    ...
    /**
     * Where the text starts, in this field's OWN space — the content box's left edge.
     *
     * <p>No {@code box().x()} term. The painter draws every box with the pose set from its own world
     * matrix, so paint coordinates start at this box's origin; and {@link #toLocal} answers in the
     * same space, which is what lets {@link #indexAt} compare a pointer against this directly. The
     * old engine's {@code screenToLocal} did not subtract the element's origin, so both halves
     * carried a {@code getX()} that cancelled out — and adding it back on one side only is the
     * off-by-a-position error that made a caret land somewhere plausible.</p>
     */
    ...
    public void paintDecoration(CgUiPaintContext ctx, Box box) {
        ...
        float offsetX = styleGen.textOffsetX().resolve(box.width());
        float offsetY = styleGen.textOffsetY().resolve(box.height());
        ...
        float originY = 0f + (box.height() - lineHeight) / 2f + offsetY;
