...
public class Scroller extends UINode {
    ...
    /** The groove. {@code scroller::part(track)} in a sheet. */
    public static final String TRACK_PART = "track";
    /** The draggable block. */
    public static final String THUMB_PART = "thumb";
     ...
     *
     * <p>A CLASS and not a part: it goes on the bar ITSELF, and a part names something a rule reaches
     * INTO a widget for. {@code scroller.__vertical__::part(thumb)} is the shape a theme writes.</p>
     */
    ...
    private void beginRepeat(float direction) {
        ...
        this.heldElapsed = 0f;
        this.repeatTimer = 0f;
        if (document() != null) document().animation().every(this, this::tickFrame);
    }

    /**
     * Runs the auto-repeat while a step button is held.
     *
     * <p>No longer has to check that the bar is still in the tree: a hook is OWNED by its node here,
     * so leaving the tree and being frozen both drop it. The old contract made that every ticker's own
     * business and named the hidden window that keeps ticking as a standing hazard.</p>
     */
     ...
     *
     * <p>INLINE origin: both are runtime state, so they must outrank the sheet's resting rule, and
     * INLINE does. It was IMPORTANT, which also works and which the new engine may not use — that
     * origin sits above every author origin including {@code !important}, so a theme could never
     * override the thumb even deliberately. A theme still controls its thickness on the cross axis,
     * its background and its states.</p>
     */
    private void applyThumb() {
        float lengthPercent = visibleRatio * 100f;
        float positionPercent = value * (100f - lengthPercent);
        StyleGroup.inlinePipeline(thumb.getStyle().getLayoutGroup(), l -> {
            if (isVertical()) {
    ...
    private float valueForThumbCentredAt(MouseEvent.Down event) {
        var local = toLocal(event.getPosition().x(), event.getPosition().y());
        var groove = track.box();
        var t = thumb.box();

        float pos = isVertical() ? local.y() : local.x();
        float start = isVertical() ? groove.y() : groove.x();
        float grooveLength = isVertical() ? groove.height() : groove.width();
        float thumbLength = isVertical() ? t.height() : t.width();
    ...
    private float travelLength() {
        var groove = track.box();
        var t = thumb.box();
        float span = isVertical() ? groove.height() - t.height() : groove.width() - t.width();
