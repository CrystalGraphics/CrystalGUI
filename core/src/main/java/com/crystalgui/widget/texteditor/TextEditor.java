...
import com.crystalgui.widget.texteditor.part.ErrorStripePart;
import com.crystalgui.widget.texteditor.part.FoldingDecorationsPart;
import com.crystalgui.widget.texteditor.part.GutterEdgePart;
import com.crystalgui.widget.texteditor.part.IndentGuidesPart;
import com.crystalgui.widget.texteditor.part.InspectionWidgetPart;
import com.crystalgui.widget.texteditor.part.LineNumbersPart;
import com.crystalgui.widget.texteditor.part.QuickFixBulbPart;
import com.crystalgui.widget.texteditor.part.RulersPart;
import com.crystalgui.widget.texteditor.part.SelectionsPart;
import com.crystalgui.widget.texteditor.part.SquigglesPart;
import com.crystalgui.widget.texteditor.part.ViewCursorsPart;
import com.crystalgui.widget.texteditor.part.WhitespacePart;
import com.crystalgui.widget.texteditor.part.ZoomIndicatorPart;
 ...
 * level up.</p>
 */
public class TextEditor extends ScrollerView implements UndoScope, DataProvider {
    /**
     * Its own kind. Every concrete node needs one, and a subclass that declares none
     * INHERITS its supertype's — so this would have reported {@code scrollerview} and matched
     ...
     */
    public static final Name NAME = Name.of("texteditor");
     ...
     */
    /**
     * This editor's own chords.
     *
     * <p>{@code bindKeys()} was an engine hook on the old element and there is none here — a node
     * declares its keymap by ANSWERING for one, so the binding happens once, at construction, and
     * {@link #keymapOrNull()} hands it to the resolver. Same shape {@code GraphView} uses, and the
     * commit that lost that on the old engine lost it silently: every command existed, was enabled,
     * showed in the palette, and answered no key at all.</p>
     */
    ...
    public DiffDecorations diffDecorations() {
    ...
    public java.util.function.IntConsumer diffRevertHandler() {
     ...
     */
    public int offsetOfPoint(TextPoint point) {
    ...
    public record RowMetrics(CursorColumns.Line line, float[] widths) {
    }
    ...
    public TextEditor revealCaretCentred() {
        ...
        setScrollNow(scrollLeft(), top - (viewportHeight() - origin - height) / 2f);
        markTreeDirty();
        return this;
    }
     ...
     */
    public boolean caretIsInView() {
        float height = lineHeight();
        float top = viewLineOf(getCaret(), LineProjection.Affinity.LEFT) * height;
        return top >= scrollTop()
                && top + height + textOriginY() <= scrollTop() + viewportHeight();
    }
    ...
    private void rememberPointer(float screenX, float screenY) {
        ...
        // NO SUBTRACTION. `toLocal` puts this element's OWN origin at zero, so its answer is
     ...
     *
     * <p>{@code clientHeightOrZero()} is the whole box: the bars are drawn <em>over</em> the content, the way
    ...
    public float viewportHeight() {
        return Math.max(0f, clientHeightOrZero() - horizontalBarThickness());
    }
     ...
     *
     * <p>Reads the previous pass's {@code widestSeen}, which is the same bargain {@link #scrollWidth()}
    ...
    private void measureScrollbars() {
        float clientWidth = clientWidthOrZero();
        float clientHeight = clientHeightOrZero();
        float contentWidth = scrollWidth();
        float barWidth = widthOf(verticalScroller());
        float barHeight = heightOf(horizontalScroller());
    ...
    private float contentHeightWithin(float viewport) {
        ...
        // for the same problem. Same split scrollHeight() makes.
     ...
     */
    /**
     * The scrollable extent on both axes.
     *
     * <p><b>One method where the old engine had {@code getScrollWidth} and {@code getScrollHeight}</b>,
     * and it is the same contract: the boxes under a virtualised view describe the WINDOW, not the
     * document, so the children genuinely cannot be asked. An editor realises a screenful of lines out
     * of a document of any length.</p>
     */
    @Override
    public float scrollExtent(boolean horizontal) {
        return horizontal ? scrollWidth() : scrollHeight();
    }
     ...
     * is {@code scrollHeight - clientHeightOrZero()}, and {@code clientHeightOrZero()} is the whole box, so the
    ...
    public int offsetAt(float screenX, float screenY) {
        var local = toLocal(screenX, screenY);
        // @see #rememberPointer -- `toLocal` is already this element's own space.
        return offsetAtLocal(local.x(), local.y());
    }
    ...
    public int offsetAtLocal(float localX, float localY) {
        ...
        float relativeX = localX + scrollLeft() - textOriginX() - carriedIndentPx(viewLine) + origin;
    ...
    public float textOriginX() {
        ...
        return paddingLeftOrZero() + leading + codeLeftPad();
    }
    ...
    public TextEditor setGutterOnRight(boolean value) {
        ...
        if (value) {
            ...
            StyleGroup.inlinePipeline(gutter.getStyle().getLayoutGroup(),
    ...
    public float gutterLeft() {
        ...
        return gutterOnRight
                ? Math.max(0f, clientWidthOrZero() - gutterWidth)
                : paddingLeftOrZero();
    }
    ...
    public EditorLanguageFeatures langFeatures() {
        return langFeatures;
    }
    ...
    public WordClassifier wordClassifier() {
    ...
    /** The completion driver. Package-private: it is the editor's, not a caller's. */
    EditorSuggest suggestions() {
        return suggest;
    }
     ...
     */
    public boolean isInCommentOrString(int offset) {
     ...
     */
    public float gutterChevronWidth() {
    ...
    public float gutterNumberWidth() {
        if (!gutterVisible) return 0f;
     ...
     *
     * <p>{@code textOriginY() + viewLine * lineHeight() - scrollTop()} was written out in eight view
     * parts and three more places here. That is a formula every part has to agree on exactly, and the
     * disagreement was already there: {@code scrollTop()} <b>can be NaN</b> — and NaN minus anything is
     ...
     */
    public float topOfViewLine(int viewLine) {
     ...
     */
    public float screenTopOfViewLine(int viewLine) {
        return topOfViewLine(viewLine) - finiteOrZero(scrollTop());
    }
     ...
     */
    public RowMetrics rowMetrics(int row) {
        var general = getStyle().getGeneralGroup();
        String fontKey = general.fontFamily() + "/" + general.fontSize();
    ...
    public TextEditor setSoftWrap(boolean value) {
        ...
        if (value) setScrollNow(0f, scrollTop());
        reproject();
        invalidateWindow();
        return this;
    }
    ...
    public TextEditor setFontSize(float size) {
        ...
        StyleGroup.inlinePipeline(getStyle().getGeneralGroup(), g -> g.fontSize(clamped));
        measuredRows.clear();
     ...
     */
    public record StableViewport(int offset, float delta) {
    ...
    public StableViewport captureFoldAnchor() {
        ...
        return new StableViewport(caret, scrollTop() - viewLine * height);
    }
    ...
    private StableViewport captureStableViewport() {
        ...
        if (!(height > 0f) || scrollTop() <= 0f) return StableViewport.NONE;
        int viewLine = Math.max(0, Math.min(viewLineCount() - 1, (int) (scrollTop() / height)));
        return new StableViewport(viewLineStartOffset(viewLine), scrollTop() - viewLine * height);
    }
    ...
    public void restoreStableViewport(StableViewport anchor) {
        ...
        setScrollNow(scrollLeft(),
                viewLineOf(anchor.offset(), LineProjection.Affinity.RIGHT) * height + anchor.delta());
        // setScroll already clamps against the box's own maxima, so the separate clamp the old engine
        // needed after every write is gone -- @see Box#setScroll.
    }
    ...
    private void syncScrollLayers() {
        final float x = -finiteOrZero(scrollLeft());
        final float y = -finiteOrZero(scrollTop());
        // THE BOX'S COMPOSITOR TRANSFORM, not the node's own scroll offset, and the three layers are
        // why: the gutter and the fold column scroll VERTICALLY ONLY, while the text scrolls both
        // ways. A scroll offset on this node would move all three together, and `scroll-exempt` is
        // all-or-nothing -- so the editor keeps doing what it always did and translates each layer
        // itself. Layout-free either way, which is what makes scrolling a 2,000-line document cost
        // one matrix rather than a reflow.
        translate(linesLayer, x, y);
        translate(gutterLayer, 0f, y);
    ...
    public float textViewportWidth() {
        ...
        return Math.max(0f, clientWidthOrZero() - textViewportLeft() - barInset - gutterInset);
    }
    ...
    private float wrapWidthPx() {
        return clientWidthOrZero() - gutterWidth - verticalBarThickness();
    }
    ...
    public void ensureCaretVisible() {
        ...
        float origin = textOriginY();
        if (!caretIsInView()) {
            if (top < scrollTop()) setScrollNow(scrollLeft(), top);
            else setScrollNow(scrollLeft(), top + height + origin - viewport);
        }
    ...
    private void revealCaretHorizontally() {
        Box selfBox = box();
        float maximum = selfBox == null ? 0f : selfBox.maxScrollLeft();
        ...
        float left = finiteOrZero(scrollLeft());
        ...
        if (wanted != left) setScrollNow(wanted, scrollTop());
    }
    ...
    /**
     * Re-derives the realised window once layout has settled.
     *
     * <p>An {@code afterLayout} hook where the old engine overrode {@code onLayoutChanged}: every
     * input is a measured box — the viewport's height, the line height, the scroll offset — and an
     * ordinary per-frame hook runs BEFORE layout, so on the frame an editor first appears it would
    ...
    /**
     * Two standing hooks, for this editor's whole life in the tree.
     *
     * <p>The heartbeat is what {@code registerTicker} was and the layout hook is what
     * {@code onLayoutChanged} was, and both are OWNED now — dropped when the editor leaves the tree
     * or is frozen, which is what makes the "hidden editor that keeps compiling" structurally
     * impossible rather than a rule the ticker had to remember.</p>
     */
    @Override
    protected void connected() {
        ...
        document.animation().afterLayout(this, this::afterLayout);
    ...
    public void updateWindow() {
        ...
        int count = viewLineCount();
        int first = Math.max(0, (int) (scrollTop() / height) - OVERSCAN);
        float viewport = viewportHeight();
    ...
    private UIElement realiseLine(int viewLine) {
        ...
        if (line == null) {
            ...
            line.append(new UIText("").addClass(SYNTAX_CLASS));
        }
        layOutLine(viewLine, line);
        if (line.parent() == null) linesLayer().append(line);
        // AND SHOWN, by the same route and for the same reason. @see #recycleLine
        // AND SHOWN, by the same route and for the same reason. @see #recycleLine
        //
        // The old engine needed TWO writes here -- the cascade candidate AND a direct poke at the
        // Taffy bridge -- because a style write reached layout through a property listener that a
        // pooled row could be recycled ahead of. There is no bridge to poke now: `setDisplayed` is
        // an attribute the box tree reads when it syncs, and it reports the structure change itself.
    ...
    private void layOutLine(int viewLine, UIElement line) {
        ...
        ((UIText) line.children().get(0)).setText(viewLineDisplayText(viewLine));
    }
    ...
    public float widthOf(int row, int column) {
        RowMetrics metrics = rowMetrics(row);
        int index = metrics.line().displayIndexOf(column);
        float[] widths = metrics.widths();
    ...
    public ProjectedLines.ModelPosition modelAt(int viewLine) {
        return projections.modelAt(viewLine);
    }

    public LineProjection projectionAt(int viewLine) {
        return projections.projectionOf(modelAt(viewLine).row());
    }

    /** The document offset at which a view line's text begins. */
    public int viewLineStartOffset(int viewLine) {
        ProjectedLines.ModelPosition model = modelAt(viewLine);
    ...
    public int viewLineEndOffset(int viewLine) {
        ProjectedLines.ModelPosition model = modelAt(viewLine);
        return buffer.document().lineStartOffset(model.row())
    ...
    public int viewLineOf(int offset, LineProjection.Affinity affinity) {
     ...
     */
    public float xOfView(int viewLine, int column) {
        ProjectedLines.ModelPosition model = modelAt(viewLine);
        LineProjection projection = projectionAt(viewLine);
    ...
    public Object getData(DataKey<?> key) {
        ...
        return undo != null ? undo : null;
    }

    // ── The box, guarded ──────────────────────────────────────────────

    /** The content box's left inset, or zero before this has a box. @see #clientWidthOrZero */
    private float paddingLeftOrZero() {
        Box box = box();
        return box == null ? 0f : box.padding().left;
    }

    /** @see #paddingLeftOrZero */
    private float paddingTopOrZero() {
        Box box = box();
        return box == null ? 0f : box.padding().top;
    }

    /**
     * A node's box dimension, or zero when it has none.
     *
     * <p>{@code Box} is nullable where {@code getRuntimeCache()} always answered, and a
     * SCROLLBAR is the case that proves it: a bar that is not shown has no box at all, so the
     * very first frame of a fresh editor threw out of {@code measureScrollbars} — which runs
    ...
    /**
     * The padding box's width, or zero while this has no box.
     *
     * <p>{@code Box} is nullable where {@code getRuntimeCache()} always answered, and the editor reads
     * its own client box in forty places — a guard at each would be forty chances to write the wrong
     * fallback. Zero is right for every one of them: an editor with no box shows no text, and every
     * consumer here is computing a width to lay glyphs into. <b>Never a divisor</b> — the sites that
     * divide already clamp with {@code Math.max}, because a NaN poisons a whole layout silently.</p>
     */
    private float clientWidthOrZero() {
        Box box = box();
        return box == null ? 0f : box.clientWidth();
    }

    /** @see #clientWidthOrZero */
    private float clientHeightOrZero() {
        Box box = box();
     ...
     * is the same one a scrollbar thumb drag uses for the same reason.</p>
     */
    private void setScrollNow(float left, float top) {
        Box box = box();
        if (box != null) box.setScroll(left, top);
    }
