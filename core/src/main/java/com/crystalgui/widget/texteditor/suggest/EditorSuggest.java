 ...
 */
public final class EditorSuggest {

    private final TextEditor editor;

    public EditorSuggest(TextEditor editor) {
        this.editor = editor;
    }
     ...
     */
    public boolean open(CompletionProvider.TriggerKind trigger, @Nullable String triggerCharacter) {
        ...
        if (window != null) {
            if (popup == null) {
                ...
                popup.onRowClicked.connect(index -> {
                    ...
                    UIDocument attached = editor.document();
                    if (attached != null) attached.focus().requestPointerFocus(editor);
                });
            }
            updateAnchor();
     ...
     */
    public static final boolean TRACE = Boolean.getBoolean("crystalgui.completion.trace");
