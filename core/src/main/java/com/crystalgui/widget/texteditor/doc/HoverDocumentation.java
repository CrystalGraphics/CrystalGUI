 ...
 * </ul>
 */
public final class HoverDocumentation {

    private final TextEditor editor;

    public HoverDocumentation(TextEditor editor) {
        this.editor = editor;
    }
    ...
    private int hoverAnchorAt(float localX, float localY) {
        ...
        float endX = editor.textOriginX() + editor.xOfView(viewLine, projection.viewLineEnd(model.viewLineInRow()))
                - finiteOrZero(editor.scrollLeft());
     ...
     */
    public void pointerForTest(int offset) {
        if (suppressed()) {
            cancel();
            return;
        }
     ...
     */
    public int hoverAnchorAtForTest(float localX, float localY) {
