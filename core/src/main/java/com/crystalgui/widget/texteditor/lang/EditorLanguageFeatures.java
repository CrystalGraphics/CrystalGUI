 ...
 */
public final class EditorLanguageFeatures {
    ...
    public EditorLanguageFeatures(TextEditor editor) {
        this.editor = editor;
     ...
     */
    public static final int LANE_BULB = 3;
     ...
     */
    public boolean requestCodeActions(int offset, Consumer<List<CodeAction>> answer) {
        return requestCodeActions(LANE_ACTIONS, offset, answer);
    }

    /** @see #LANE_BULB for why the gutter bulb asks on a lane of its own. */
    public boolean requestCodeActions(int lane, int offset, Consumer<List<CodeAction>> answer) {
    ...
    public HoverDocumentation hover() {
        return hover;
    }
     ...
     */
    public boolean showDocumentationAt(int offset) {
        UIDocument window = editor.document();
        if (window == null) return false;
        float[] anchor = editor.anchorInWindow(offset);
        if (anchor == null) return false;
