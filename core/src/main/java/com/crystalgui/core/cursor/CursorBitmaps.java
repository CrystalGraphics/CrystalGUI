 ...
 *
 * <p><b>It also owns the keyword table</b>: {@link #artFor} answers which picture a {@link Cursor} wants,
 * and that is the single copy. A platform adapter never enumerates keywords — it asks, caches the native
 * object it builds against the returned {@link CursorArt}, and therefore needs no edit when a cursor is
 * added. See {@link CursorArt} for what the copies-per-adapter arrangement cost.</p>
 *
 * <p>Adding a cursor is two edits and both are here: a {@link Cursor} constant, and a row in
 * {@link #artFor} pointing at either an existing {@link CursorArt} or a new one with its drawing.</p>
 *
 *
...
public final class CursorBitmaps {
    ...
    private static final CursorArt VERTICAL_ARROW   = CursorArt.centred("vertical-arrow",   CursorBitmaps::verticalDoubleArrow);
    private static final CursorArt DIAGONAL_NWSE    = CursorArt.centred("diagonal-nwse",    CursorBitmaps::diagonalNwseArrow);
    private static final CursorArt DIAGONAL_NESW    = CursorArt.centred("diagonal-nesw",    CursorBitmaps::diagonalNeswArrow);
    ...
    /** The one asymmetric picture: a hand points, so the click lands on the fingertip. */
    private static final CursorArt POINTING_HAND = new CursorArt(
            "pointing-hand", CursorBitmaps::pointingHand, HAND_HOTSPOT_X, HAND_HOTSPOT_Y);

    /** No bitmap — GLFW's own crosshair is better than one we would draw. @see CursorArt#standardOnly */
    private static final CursorArt CROSSHAIR = CursorArt.standardOnly("crosshair");

    private static final Map<Cursor, CursorArt> ART = new EnumMap<>(Cursor.class);

    static {
        art(HORIZONTAL_ARROW, Cursor.EW_RESIZE, Cursor.COL_RESIZE, Cursor.E_RESIZE, Cursor.W_RESIZE);
    ...
    private static void art(CursorArt art, Cursor... keywords) {
        for (Cursor keyword : keywords) ART.put(keyword, art);
    }

    /**
     * Which picture a keyword wants, or {@code null} when nothing here draws it.
     *
     * <p>{@code null} is the common answer and is deliberate: {@link Cursor} carries all of CSS's
     * keywords while only a handful matter to this UI, and the system arrow beats a wrong picture.</p>
     */
    public static @Nullable CursorArt artFor(Cursor cursor) {
        return ART.get(cursor);
    }
    ...
    /**
     * ◄ ► — two opposed triangles with a clear gap between them. {@link Cursor#SLIDE_ARROW}: this
