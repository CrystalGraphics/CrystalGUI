...
/**
 * A tool document is part of the document it belongs to, not a peer of it — Win32's {@code WS_EX_TOOLWINDOW}.
 *
 * <p>What these pin is <b>citizenship</b>, which is three separate facts that have to agree: a tool
 * document is in no taskbar, in no switcher, and goes away when the thing it belongs to does. Each was
 ...
 * would pass half of these, so every case also names a document that must still be a citizen.</p>
 */
public class ToolWindowIsNotACitizenTest extends UiDocumentTestBase {
    ...
    /** Opens the Inspector as a top-level tool document, owned by the workbench's frame. */
    private ToolWindowFrame openWindowed() {
        ...
        assertNotNull("the fixture did not open a windowed tool document", frame);
        return frame;
    }
    ...
    /**
     * <b>It is a live document, and it is in neither of the lists a live document is normally shown in.</b>
     *
    ...
    public void aWindowedToolWindowIsLiveButIsNotAnEntry() {
        ...
        var registry = Desktop.of(document).registry();

        assertTrue("a tool document is still a live document", registry.windows().contains(frame));
        assertFalse("a tool document must not have a taskbar entry",
                registry.taskbarOrder().contains(frame));
        assertFalse("a tool document must not be offered by the switcher",
                registry.switcherOrder().contains(frame));

        // ...and the filter is not simply eating everything.
        assertTrue("an ordinary document lost its taskbar entry",
                registry.taskbarOrder().contains(workbenchWindow));
        assertTrue("an ordinary document lost its place in the switcher",
     ...
     *
     * <p>This is the torn-out editor case in miniature: a document that came out of another document and
    ...
    public void anOwnedWindowThatIsNotAToolWindowKeepsItsEntry() {
        WindowFrame torn = Desktop.of(document).addWindow(new WindowFrame("Torn-out editor"));
        torn.setOwnerWindow(workbenchWindow);
        settle();

        assertFalse("the fixture accidentally made it a tool document", torn.isToolWindow());
        assertTrue("an owned NON-tool document must keep its taskbar entry",
                Desktop.of(document).registry().taskbarOrder().contains(torn));
    }
     ...
     *
     * <p>Win32's owner/owned rule. A {@code FLOATING} tool document gets this for nothing by being a child
    ...
    public void hidingTheOwnerHidesItsToolWindowAndShowingBringsItBack() {
        ...
        settle();
        assertEquals("minimising the workbench left its tool document on screen",
                WindowState.HIDDEN, frame.state());

        workbenchWindow.show(true);
        settle();
        assertEquals("restoring the workbench did not bring its tool document back",
                WindowState.VISIBLE, frame.state());
    }

    /**
     * <b>...but a tool document the user had ALREADY put away stays away.</b>
     *
     ...
     * survives a hide is the placement record and not the document. So the frame's state after a real
     ...
     * On screen the document sailed into the taskbar and its panels blinked out of existence once it
    ...
    public void minimisingTheOwnerAnimatesItsToolWindowsOut() {
        ...
        // document's own OPEN timeline is still running when the gesture starts, so isAnimating() answers
        ...
        try {
            ...
            assertTrue("the tool document did not start its own flight when the owner was minimised",
                    frame.isAnimating());
            assertEquals("the tool document was detached instead of being animated out",
                    WindowState.VISIBLE, frame.state());
        } finally {
    ...
    public void theOwnersOwnHideDoesNotCutShortTheFlightItStarted() {
        ...
        try {
            ...
            assertEquals("the owner's hide detached a tool document that was still flying",
                    WindowState.VISIBLE, frame.state());
            assertTrue("the tool document's flight was cancelled", frame.isAnimating());
        } finally {
            Desktop.setAnimationsEnabled(false);
        }
    }
     ...
     * — it calls {@code hidePanel}, which drops the frame and records the tool document shut. Firing it
     * during a cascade would mean minimising the workbench marked every panel on it closed, and the very
     * next session save would write that down: the document comes back, the panels do not. Asserted
    ...
    public void aCascadeDoesNotRecordThePanelAsClosed() {
        ...
        workbenchWindow.hide();
        settle();
        assertTrue("minimising the workbench recorded its tool document as closed",
                manager.isPanelOpen(INSPECTOR));
     ...
     * frame's own Hide button would take the document off screen and leave the manager believing the panel
    ...
    public void hidingAToolWindowDirectlyStillClosesThePanel() {
        ...
        assertFalse("hiding a tool document by hand did not record the panel closed",
                manager.isPanelOpen(INSPECTOR));
    }
}
