 ...
 */
public class ProblemsPanelTest extends UiDocumentTestBase {
    ...
    public void foldStateReachesTheRowAsAClass() {
        ...
        assertTrue("an expanded file must say so",
                heading.hasClass(com.crystalgui.widget.collection.tree.TreeView.EXPANDED_CLASS));
    }
    ...
    private void chooseViewOption(String label) {
        for (com.crystalgui.widget.overlay.MenuItem item : panel.viewMenu().getItems()) {
    ...
    public void anEmptyTreeSaysWhichKindOfEmptyItIs() {
        ...
        assertTrue("a filtered-out tree claimed the workspace was clean",
                ((com.crystalgui.widget.text.UIText) empty).getText().toLowerCase().contains("filter"));
    }
     ...
     * document — because a collapsed heading shows a filename and a count, which is what the status bar
     ...
     * used to blur whatever had focus and then focus nothing at all, leaving the whole document with
    ...
    public void foldingAHeadingLandsFocusOnTheRow() {
        ...
        // deferred to the next frame. One frame is enough to see — every :focus ring in the document goes
        ...
        for (int frame = 0; frame < 8; frame++) {
            frame();
            assertNotNull("frame " + frame + " had no focus owner at all — every ring in the document blinks,"
        ...
        assertSame("focus should land on the row the chevron belongs to",
                headingRow, document.focus().focused());
    }
    ...
    public void collapsingMovesFocusToTheHeadingAndUnfoldingKeepsItThere() {
        ...
        for (int frame = 0; frame < 6; frame++) {
            ...
            assertNotNull("folding left frame " + frame + " with no focus owner — the arrows cannot walk"
                    + " back out of what was just collapsed", document.focus().focused());
        }
    ...
    public void doubleClickingAProblemReportsItAndSelectsIt() {
        ...
        assertTrue("the row was never marked selected, so nothing can highlight",
                problemRow.hasClass(com.crystalgui.widget.collection.list.ListView.SELECTED_CLASS));
