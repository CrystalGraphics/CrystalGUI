 ...
 *
 * <p>Ranking lives in {@code QuickPickSourceTest} and is headless. What needs a document is the half that is
 ...
 */
public class CommandPaletteTest extends UiDocumentTestBase {

    /** Physical pixels per logical pixel — {@code UIDocument}'s default. */
    ...
    public void setUpWindow() {
        ...
        // default.css, so a document without it exercises a palette with no stylesheet at all. The geometry
     ...
     * inside its scope.
     *
     * <p>Opening a palette moves focus into the palette's own search field, which sits under the document
    ...
    public void aScopedCommandIsListedWhenThePaletteWasOpenedFromInsideItsScope() {
        ...
        QuickPick pick = CommandPalette.open(document);
     ...
     * reachable: it is exactly the state an untouched document opens in. */
    @Test
    ...
    public void openingFocusesTheSearchField() {
        QuickPick pick = CommandPalette.open(document);
        assertSame(pick.searchField().field(), document.focus().focused());
    }
    ...
    public void closingDetachesThePaletteFromTheTree() {
        QuickPick pick = CommandPalette.open(document);
        assertNotNull(pick.parent());

        pick.hide();

        assertEquals(null, pick.parent());
    }
    ...
    public void aBoundCommandShowsItsAccelerator() {
        document.keymap().bind("Mod+Shift+G", GLOBAL);
        document.focus().requestFocus(inner);
    ...
    public void aCommandThatNeededTheFocusedElementIsMarkedContextual() {
        ...
        List<QuickPickItem> items = CommandPalette.itemsFor(document.getCommands(), inner);
     ...
     * "anywhere" with an empty context takes the document's own providers away too — and those are exactly
    ...
    public void aCommandTheWindowAnswersForIsNotContextual() {
        ...
        document.addDataProvider(asked -> asked == key ? "yes" : null);
        document.getCommands().register(Command.of(WINDOW_SCOPED, "Window Action")
        ...
        assertTrue("the document's provider was not consulted at all",
                itemFor(items, WINDOW_SCOPED).enabled());
        assertFalse("a document-scoped command was measured against an empty context",
     ...
     * document-scoped command would report itself unavailable there and be marked contextual, which is the
    ...
    public void nothingIsContextualWhenThePaletteWasOpenedAtTheRoot() {
        List<QuickPickItem> items =
                CommandPalette.itemsFor(document.getCommands(), document);
