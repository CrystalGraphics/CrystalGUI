 ...
 */
public class ListViewTest extends UiDocumentTestBase {
    ...
    /** Style + tickers + layout, which is what drives the document — see ListView.tickFrame. */
    private void settle() {
    ...
    public void scrollingRecyclesRatherThanAllocating() {
        ...
        for (int step = 1; step <= 50; step++) {
            list.scrollTo(list.scrollLeft(), step * 40f);
            settle();
        }

        // Not equal to the count at rest: at the very top the leading overscan is clipped by index 0, so
        // the document is two rows smaller there than in the interior. Bounded is the property that matters.
        assertTrue("document grew to " + list.realisedCount(), list.realisedCount() <= realisedBefore + 2);
    ...
    public void scrollHeightComesFromTheModelNotTheChildren() {
        build(1_000);
        assertEquals(10_000f, list.box().scrollHeight(), 0.01f);
        assertEquals(10_000f - list.box().clientHeight(), list.box().maxScrollTop(), 0.01f);
    }
    ...
    public void templatesAreCreatedOncePerElementAndBoundPerRow() {
        ...
        list.scrollTo(list.scrollLeft(), 5_000f);
        settle();
        int templatesAfterWarmup = templatesCreated;
        // Now the pool has reached its steady size (the interior document is two larger than the one at the
        // very top, where overscan is clipped), so from here on scrolling must create nothing at all.
        for (int i = 1; i <= 20; i++) {
            list.scrollTo(list.scrollLeft(), 5_000f + i * 130f);
            settle();
        }
    ...
    public void aModelChangeUpdatesTheView() {
        build(20);
        float heightBefore = list.box().scrollHeight();

        model.add("one more");
        settle();

        assertEquals(heightBefore + 10f, list.box().scrollHeight(), 0.01f);
    }
    ...
    public void anEmptyModelRealisesNothing() {
        ...
        assertEquals(0f, list.box().scrollHeight(), 0.01f);
        assertEquals("and cannot scroll", 0f, list.box().maxScrollTop(), 0.01f);
    }
    ...
    public void focusFollowsTheIndexAcrossRecycling() {
        ...
        list.scrollTo(list.scrollLeft(), 8_000f);
        settle();
     ...
     * {@code Input} correctly forgets it, and the next refresh of the tree pocketed the vacancy.
    ...
    public void aRecycledRowDoesNotCarryFocusToItsNextItem() {
        build(10_000);
        document.focus().requestFocus(list.realisedRows().get(5));
        settle();
    ...
    @Test
    public void theListIsContentAndItsRowsAreNot() {
        build(10);
        // The old engine flagged each row an INTERNAL child; there is no such flag here. What
        // survives is the distinction it was drawing: the list is an ordinary node a caller
        // places, and its rows belong to the widget -- realised from the model, recycled, and
        // never described to a peer, which would otherwise be sent whichever dozen of ten
        // thousand happened to be on screen.
        assertFalse("a list is content, not a part", list.get(Attribute.PART).isEmpty() == false);
        assertFalse("rows exist", list.realisedRows().isEmpty());
        assertTrue("but they are the widget's, not the list's content",
                list.describedChildren().isEmpty());
    }
    ...
    private void clickRow(int index) {
        ...
        var centre = com.crystalgui.core.data.Transform2D.apply(cache.localToWorld(),
                cache.width() * 0.5f, cache.height() * 0.5f);
        int x = Math.round(centre.x()), y = Math.round(centre.y());
        document.input().consumeMouseEvent(new com.crystalgraphics.platform.input
    ...
    public void restoringFocusAfterRecyclingDoesNotDisturbTheSelection() {
        ...
        list.select(2);
        document.focus().requestFocus(list.realisedRows().get(2));
        settle();
        list.toggle(3);
        list.toggle(4);
        assertEquals(java.util.Set.of(2, 3, 4), list.getSelectedIndices());
     ...
     * nothing at all before this: {@code Input} turns Space/Enter into a synthesized click on
    ...
    public void aZeroRowHeightIsRejected() {
        try {
            ...
            fail("zero would put every row at the same offset — the whole model in one document");
     ...
     * scrolled out of the realised document, pulling the thumb and the content sideways under the pointer
    ...
    public void horizontalScrollingMeasuresTheWidestRealisedRow() {
        ...
        assertTrue("the long row sets the range, not the viewport: " + wide.box().scrollWidth(),
                wide.box().scrollWidth() >= 400f);
        assertTrue("so there is somewhere to scroll to", wide.box().maxScrollLeft() > 0f);

        wide.box().setScroll(0f, 5_000f);
        for (int i = 0; i < 6; i++) frame();

        assertTrue("the range survives the long row leaving the realised document: " + wide.box().scrollWidth(),
                wide.box().scrollWidth() >= 400f);
    }
    ...
    public void withoutHorizontalScrollingRowsNeverOutgrowTheViewport() {
        ...
        assertEquals("rows are written to the viewport width, so there is nothing to scroll",
                0f, list.box().maxScrollLeft(), 0.01f);
    }
    ...
    public void recyclingIsAnnouncedWhileItHappens() {
        ...
        // Far enough that every realised row leaves the document and is pooled.
        list.scrollTo(list.scrollLeft(), 900f);
        settle();
