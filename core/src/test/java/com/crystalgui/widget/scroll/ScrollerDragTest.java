 ...
 */
public class ScrollerDragTest extends UiDocumentTestBase {
    ...
    private ScrollerView setUp(float uiScale) {
        ...
        // each time, and `dragResultIsIndependentOfUiScale` calls this twice.
        document.removeAll();
    ...
    /**
     * The thumb's centre in SURFACE pixels, which is what the input entry points take.
     *
     * <p>{@code worldX()} rather than {@code x()}: the box's own offset is PARENT-RELATIVE here,
     * where the old engine's runtime cache was absolute -- and a thumb sits at 0 inside its own
     * scroller, so the ported arithmetic pressed at (4,50) and every drag test failed as though
     * the thumb were not hittable. The world position already has the root transform baked in, so
     * only the HALF-EXTENT is scaled; scaling the origin too multiplies it a second time.</p>
     */
    private int[] thumbCentrePhys(float uiScale) {
        var t = view.verticalScroller().thumb().box();
        return new int[]{
                Math.round(t.worldX() + t.width() / 2f * uiScale),
                Math.round(t.worldY() + t.height() / 2f * uiScale)
        };
    }
    ...
    public void wheelDownScrollsContentDown() {
        setUp(2f);
        var c = view.box();
        int x = Math.round((c.x() + 20f) * 2f), y = Math.round((c.y() + 20f) * 2f);
        ...
        assertTrue("wheel-down should increase scrollTop, got " + view.scrollTop(),
                view.scrollTop() > 0f);
    }
    ...
    public void wheelUpScrollsBackToTheStart() {
        ...
        view.scrollTo(view.scrollLeft(), 80f);
        mouseTo(x, y);
        frame();
        wheel(x, y, -1f);
        frame();

        assertTrue("wheel-up should decrease scrollTop", view.scrollTop() < 80f);
    }
    ...
    public void wheelIsClamped() {
        ...
        settle();
        assertEquals(view.box().maxScrollTop(), view.scrollTop(), 0.5f);
    ...
    public void wheelEasesTowardTheTargetInsteadOfJumping() {
        ...
        assertTrue("scroll jumped straight to the target instead of easing"
                        + " (at " + view.scrollTop() + " of " + target + ")",
                view.scrollTop() < target);
        assertTrue("scroll did not move at all", view.scrollTop() > 0f);

        settle();
        assertEquals("smooth scroll never reached its target", target, view.scrollTop(), 0.5f);
    }
    ...
    public void autoBehaviourJumpsImmediately() {
        ...
        view.scrollTo(view.scrollLeft(), 60f);
        assertEquals("scroll-behavior:auto should not animate", 60f, view.scrollTop(), 0.5f);
    }
    ...
    public void draggingTheThumbIsNotSmoothed() {
        ...
        float travel = view.verticalScroller().box().height()
                - view.verticalScroller().thumb().box().height();
    ...
    public void pressOnTheThumbStartsADrag() {
        ...
        var thumb = view.verticalScroller().thumb().box();
    ...
    public void draggingTheThumbScrollsTheContent() {
        ...
        assertTrue("dragging the thumb did not scroll, scrollTop=" + view.scrollTop(),
                view.scrollTop() > 0f);
    ...
    public void thumbStaysInsideTheTrackAtBothExtremes() {
        ...
        view.scrollTo(view.scrollLeft(), 0f);
        frame();
        assertTrue(bar.thumb().box().y() >= bar.box().y() - 0.5f);

        view.scrollTo(view.scrollLeft(), view.box().maxScrollTop());
        frame();
        var t = bar.thumb().box();
        var track = bar.box();
        assertTrue("thumb overhangs the track at max scroll",
                t.y() + t.height() <= track.y() + track.height() + 0.5f);
    }
    ...
    public void bothBarsLeaveTheCornerClear() {
        setUpBothAxes();
        var v = view.verticalScroller().box();
        var h = view.horizontalScroller().box();

        assertTrue("both bars should be visible", v.height() > 0f && h.width() > 0f);

        float vBottom = v.y() + v.height();
        float hTop = h.y();
    ...
    public void clickingTheTrackCentresTheThumbOnTheClick() {
        setUp(2f);
        Scroller bar = view.verticalScroller();
        var groove = bar.track().box();

        // Three-quarters down the groove.
        // The assertion below compares two boxes that share a parent, so it stays in the box's own
        // parent-relative space. Only the PRESS POINT has to reach surface pixels, and `worldY()`
        // already carries the scale -- so the fraction of the groove takes it and the origin does not.
        float targetLocalY = groove.y() + groove.height() * 0.75f;
        int px = Math.round(groove.worldX() + groove.width() / 2f * uiScale());
        int py = Math.round(groove.worldY() + groove.height() * 0.75f * uiScale());
        ...
        assertTrue("a track click should ease, not jump",
                view.scrollTop() < view.getTargetScrollTop());

        settle();
        var thumb = bar.thumb().box();
        float thumbCentre = thumb.y() + thumb.height() / 2f;
    ...
    public void clickingTheEndOfTheTrackPinsToTheExtreme() {
        ...
        int px = Math.round(groove.worldX() + groove.width() / 2f * uiScale());
        int py = Math.round(groove.worldY() + (groove.height() - 1f) * uiScale());
        ...
        assertEquals(view.box().maxScrollTop(), view.scrollTop(), 1f);
    }
    ...
    public void stepButtonsExistAndAreHiddenByDefault() {
        ...
        assertNull("head button should be display:none by default -- and a hidden node has NO"
                + " box at all here, which is a stronger statement than a zero-height one",
                bar.head().box());
        assertNull(bar.tail().box());
    }
    ...
    public void stepIsOneLineRegardlessOfContentLength() {
        setUp(2f);
        float shortStepPx = view.verticalScroller().getStepFraction() * view.box().scrollHeight();
        ...
        view.refreshScrollers();
        frame();
        float longStepPx = view.verticalScroller().getStepFraction() * view.box().scrollHeight();
    ...
    public void holdingAStepButtonKeepsScrolling() {
        ...
        for (int i = 0; i < 10; i++) frame();   // 0.16s < 0.3s delay
    ...
    public void stepButtonsAnimateWhileDraggingDoesNot() {
        ...
        settle();
        assertEquals(view.getTargetScrollTop(), view.scrollTop(), 0.5f);
    }
    ...
    public void showingButtonsShrinksTheTrack() {
        setUp(2f);
        Scroller bar = view.verticalScroller();
        float trackBefore = bar.track().box().height();

        document.styleEngine().addStylesheet(StyleSheet.parse(
                "scroller::part(head), scroller::part(tail) { display: flex; height: 12px; }"));
        frame();
    ...
    public void barsHideWhenContentFits() {
        ...
        assertEquals(0f, view.box().maxScrollTop(), 0.5f);
        assertNull("bar should be display:none when content fits, and a hidden node has no box",
                view.verticalScroller().box());
    }
    ...
    public void aPlainWheelScrollsAHorizontalOnlyView() {
        horizontalOnly();
        assertEquals("precondition: nothing to scroll vertically", 0f, view.box().maxScrollTop(), 0.5f);
        assertTrue("precondition: something to scroll horizontally", view.box().maxScrollLeft() > 0f);
    ...
    public void aPlainWheelStaysVerticalWhenThereIsAVerticalAxis() {
        ...
        assertEquals("the horizontal axis must not move", 0f, view.scrollLeft(), 0.5f);
    }
    ...
    public void hiddenScrollbarsStillScroll() {
        ...
        assertNull("the bar must be gone", view.verticalScroller().box());

        wheel(20, 20, 1f);
        settle();

        assertTrue("hiding the bars must not disable scrolling", view.scrollTop() > 0f);
    }
}
