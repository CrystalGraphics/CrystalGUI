 ...
 */
public class GraphEditingTest extends UiDocumentTestBase {
    ...
    public void aTouchedNodeRisesAndStaysRisen() {
        ...
        assertTrue("the later node starts on top",
                isInside(hit(overlap.x(), overlap.y()), over));
    ...
    public void aWireCanBePickedAndDeleted() {
        ...
        // IN THE WIRE LAYER'S SPACE, which is what `pickWire` takes. `dotCenterIn(space)` says
        // which space it is answering in; subtracting the layer's own `box().x()` from a world
        // answer does not, because `Box.x()` is parent-relative and the two are not the same
        // conversion. `Box.centreIn` is the one that carries the transforms between them.
        Vector2f from = out.dotCenterIn(graph.wireLayer());
        Vector2f to = in.dotCenterIn(graph.wireLayer());
        var picked = graph.pickWire((from.x() + to.x()) / 2f, (from.y() + to.y()) / 2f);
    ...
    public void deleteRemovesASelectedWireFromTheKeyboard() {
        ...
        Vector2f from = a.getOutputPorts().get(0).dotCenterIn(graph.wireLayer());
        Vector2f to = b.getInputPorts().get(0).dotCenterIn(graph.wireLayer());
        Vector2f onWire = graph.worldToViewport((from.x() + to.x()) / 2f,
                (from.y() + to.y()) / 2f);
        Vector2f physical = Transform2D.apply(graph.box().localToWorld(), onWire.x(), onWire.y());
