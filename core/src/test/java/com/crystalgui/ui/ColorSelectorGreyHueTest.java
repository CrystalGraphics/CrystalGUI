 ...
 */
public class ColorSelectorGreyHueTest extends UiTestBase {
    ...
    private UIElement part(String cssClass) {
        return picker.querySelectorAll("." + cssClass).get(0);
    }
    ...
    private void pressRingAt(float hue) {
        ...
        var cache = ring.getRuntimeCache();
        ...
        float localX = cache.getX() + cache.getWidth() * (0.5f + offset[0]);
        float localY = cache.getY() + cache.getHeight() * (0.5f + offset[1]);
        var world = Transform2D.apply(cache.localToWorld.get(), localX, localY);

        var handler = window.getInputHandler();
        handler.sendInputEvent(ring, new MouseEvent.Down(ring,
    ...
    private float handleX() {
        return part(ColorSelector.RING_HANDLE_CLASS).getRuntimeCache().getX();
    }
