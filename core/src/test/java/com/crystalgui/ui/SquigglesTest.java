 ...
 *
 * <p>The model is covered headlessly by {@code DiagnosticSetTest}. What needs a window is the part that
 ...
 */
public class SquigglesTest extends UiTestBase {
    ...
    private static void collect(UIElement element, String severityClass, List<UIElement> out) {
        if (element.hasClass("__squiggle__") && element.hasClass(severityClass)
                && element.getRuntimeCache().getHeight() > 0f) {
            out.add(element);
        }
        for (UIElement child : element.getChildren()) collect(child, severityClass, out);
    }
    ...
    public void aZeroWidthDiagnosticStillDrawsAVisibleBand() {
        ...
        assertTrue("a zero-width band is invisible", found.get(0).getRuntimeCache().getWidth() >= 1f);
    }
