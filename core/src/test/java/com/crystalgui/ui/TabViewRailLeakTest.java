 ...
 */
public class TabViewRailLeakTest extends UiTestBase {
    ...
    public void clearingTabContentFirstStillLetsClearTabsWork() {
        TabView view = new TabView();
        view.addTab("one").content().addChild(new UIElement());
        view.addTab("two").content().addChild(new UIElement());

        for (Tab tab : view.getTabs()) tab.content().clearAllChildren();
        view.clearTabs();
        view.addTab("only");
