 ...
 */
public class ErrorStripeTest extends UiTestBase {
    ...
    private static void collect(UIElement element, List<UIElement> out) {
        if (element.hasClass("__error-stripe__") && element.getRuntimeCache().getHeight() > 0f) {
            out.add(element);
        }
    ...
    private float trackHeight() {
        return editor.verticalScroller().track().getRuntimeCache().getHeight();
    }
    ...
    public void aMarkSitsProportionallyDownTheGrooveAtItsProblemsPosition() {
        ...
        float top = found.get(0).getRuntimeCache().getY();
        float middle = found.get(1).getRuntimeCache().getY();
        float bottom = found.get(2).getRuntimeCache().getY();
        ...
        assertTrue("the last problem's mark is not in the lower half of the groove",
                bottom > editor.verticalScroller().track().getRuntimeCache().getY() + trackHeight() / 2f);
    }
    ...
    public void aMarkOnTheFinalRowStaysInsideTheGroove() {
        ...
        List<UIElement> found = marks();
        assertEquals(1, found.size());
        float trackBottom = editor.verticalScroller().track().getRuntimeCache().getY() + trackHeight();
        var cache = found.get(0).getRuntimeCache();
        assertTrue("the mark hangs off the bottom of the groove",
                cache.getY() + cache.getHeight() <= trackBottom + 0.5f);
        assertTrue("the mark has no height", cache.getHeight() > 0f);
    }
    ...
    public void aMarkPaintsAboveTheScrollbarThumb() {
        ...
        UIElement thumb = findByClass(editor.verticalScroller(), "__thumb__");
