 ...
 */
public class FrameThreadOwnershipTest extends UiTestBase {

    /** Builds a window and paints it once, so this thread owns its tree. */
    private UIWindow paintedWindow(UIElement root) {
    ...
    public void mutatingAPaintedTreeOffItsFrameThreadIsRefused() throws Exception {
        ...
        assertTrue("the message must name the operation, was: " + raised[0].getMessage(),
                raised[0].getMessage().contains("Adding a child"));
    }
