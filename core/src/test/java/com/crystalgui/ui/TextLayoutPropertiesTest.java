 ...
 */
public class TextLayoutPropertiesTest extends UiTestBase {
    ...
    public void normalWhiteSpaceWrapsWithinMaxWidth() {
        ...
        assertTrue("should wrap inside its max-width", text.getRuntimeCache().getWidth() <= 80.5f);
        assertTrue("and be several lines tall, was " + text.getRuntimeCache().getHeight(),
    ...
    public void nowrapProducesOneLineThatOverflowsTheBox() {
        ...
        assertTrue("must collapse to a single line, was " + text.getRuntimeCache().getHeight(),
                text.getRuntimeCache().getHeight() < 40f);
        assertTrue("the box stays capped by max-width; the text overflows it, was "
                        + text.getRuntimeCache().getWidth(),
    ...
    public void whiteSpaceInherits() {
        ...
        assertTrue("an inherited nowrap must actually stop the wrapping, height was "
                        + child.getRuntimeCache().getHeight(),
                child.getRuntimeCache().getHeight() < 40f);
    }
    ...
    public void ellipsisOnlyAppliesWhenTextDoesNotWrap() {
        ...
        assertTrue(wrapping.getRuntimeCache().getWidth() <= 80.5f);
        assertTrue(wrapping.getRuntimeCache().getHeight() > 40f);
    }
