 ...
 */
public class QuickPickResizeTest extends UiTestBase {
    ...
    private static float bottomOf(UIElement element) {
        return element.getRuntimeCache().getY() + element.getRuntimeCache().getHeight();
    }
    ...
    public void aResizedPopupUsesTheHeightItWasGiven() {
        ...
        float listHeight = pick.resultList().getRuntimeCache().getHeight();
    ...
    public void anUntouchedPopupStillShrinksToItsRows() {
        ...
        float popupHeight = pick.getRuntimeCache().getHeight();
        assertTrue("an unresized popup grew to fill its window: " + popupHeight,
                popupHeight < 400f);
    }
}
