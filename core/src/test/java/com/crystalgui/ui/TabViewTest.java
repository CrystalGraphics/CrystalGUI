...
public class TabViewTest extends UiTestBase {
    ...
    /** Full pass — only safe before any Tab exists. See {@link #styleFrame()}. */
    private void frame() {
    ...
    public void theTabViewRefusesArbitraryChildren() {
        try {
            tabView.addChild(new UIElement());
    ...
    public void panesLiveInThePanesContainerNotUnderTheTab() {
        ...
        assertTrue(tabView.panes().hasChild(tab.content()));
        assertFalse(tab.hasChild(tab.content()));
    }
    ...
    public void theStripHoldsARailAndALaidOutBar() {
        ...
        assertTrue("tabs belong to the rail", tabView.rail().hasChild(tab));
        assertTrue(tabView.strip().hasChild(tabView.rail()));
        assertTrue("the bar is a sibling of the rail, not an overlay inside it",
                tabView.strip().hasChild(tabView.bar()));
