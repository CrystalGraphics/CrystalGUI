 ...
 */
public class MenuBarViewTest extends UiTestBase {
    ...
    private UIElement titleAt(int index) {
        ...
        for (UIElement child : bar.getChildren()) {
    ...
    private void press(UIElement element) {
        window.getInputHandler().sendInputEvent(element,
                new MouseEvent.Down(element, ORIGIN, 0, 1));
    }

    private void hover(UIElement element) {
        window.getInputHandler().sendInputEvent(element, new MouseEvent.Enter(element, ORIGIN));
    }
    ...
    public void closingDetachesEveryMenuInTheChain() {
        press(titleAt(0));
        int attached = countMenus(window.ui.rootElement);
        ...
        assertEquals("left in place they accumulate one set per press",
                0, countMenus(window.ui.rootElement));
    }
    ...
    private void rawPress(UIElement target) {
        ...
        org.joml.Vector2f centre = com.crystalgui.core.data.Transform2D.apply(cache.localToWorld.get(),
    ...
    private Menu openMenuElement() {
        return findMenu(window.ui.rootElement);
    }
    ...
    public void aLetterWithoutAltIsNotAMnemonic() {
        press(titleAt(0));
        bar.close();
        window.getInputHandler().sendInputEvent(root,
    ...
    private void altPress(char letter) {
        window.getInputHandler().sendInputEvent(root, new com.crystalgui.ui.event.KeyboardEvent.Down(
    ...
    public void collapsingHidesTheTitlesAndShowsTheBurger() {
        ...
        assertEquals("a collapsed title takes no space", 0f,
                titleAt(1).getRuntimeCache().getWidth(), 0.01f);
    }
    ...
    private static int separatorsIn(Menu menu) {
        ...
        for (UIElement child : menu.itemsContainer().getChildren()) {
            if (child.hasClass(Menu.SEPARATOR_CLASS)) count++;
        }
        return count;
    }
    ...
    private static int countMenus(UIElement element) {
        ...
        for (UIElement child : element.getChildren()) count += countMenus(child);
        return count;
    }
}
