...
public class ShaderGraphCommandsTest extends UiTestBase {
    ...
    public void aBareGraphBindsUndoUnderTheSharedId() {
        ...
        // a binding -- which is what removed the last reason a widget needed a window to install one.
        assertNotNull("Ctrl+Z is not bound anywhere",
                window.getCommands().declaredBindings().chordFor(UndoCommands.UNDO));
        assertNotNull("Ctrl+Shift+Z / Ctrl+Y is not bound anywhere",
                window.getCommands().declaredBindings().chordFor(UndoCommands.REDO));
    }
    ...
    public void theBareLetterKeysAreScopedToTheGraphRatherThanTheWindow() {
        ...
        assertNull("a bare letter must not be bound at the window root, or it fires while typing",
        ...
        assertTrue("a bare letter should apply with focus inside the graph",
                window.getCommands().get(GraphCommands.FRAME_ALL)
                        .isEnabled(CommandContext.of(graph)));
        assertFalse("a bare letter must not apply at the window root",
                window.getCommands().get(GraphCommands.FRAME_ALL)
                        .isEnabled(CommandContext.of(
                                window.ui.rootElement)));
    }
    ...
    public void theAssembledShaderEditorGetsThemThroughItsGraph() {
        ...
        assertNotNull(window.getCommands().get(GraphCommands.DELETE));
        assertNotNull(window.getCommands().get(UndoCommands.UNDO));
    }
