 ...
 *
 * <p>{@code gap} did not. {@code BoxStyle} reset the bridge by setting {@code gap} to ZERO before
 * applying what the cascade held, and the bridge resolves an axis as "the longhand if it is not auto,
 * else {@code gap} if it is not auto, else {@code gap}" — so a non-auto reset made the last branch
 * unreachable. The property parsed, cascaded and answered {@code isSet}, and laid out at zero.</p>
 *
 * <p>Which is why every shipped sheet spells it {@code gap}: the one rule that reached for the CSS
...
public class GapShorthandTest extends UiDocumentTestBase {
    ...
    public void everyGapSpellingSpacesARow() {
        ...
        assertEquals("gap", 40f, secondChildX(g -> g.gapAll(10f)), 0.01f);
        assertEquals("column-gap", 40f, secondChildX(g -> g.gapColumn(10f)), 0.01f);
    }

    /** A withdrawn gap is withdrawn — only `gap` was reset before, so a longhand lingered. */
    @Test
