 ...
 */
public class SplitViewNAryTest extends UiTestBase {
    ...
    /**
     * The element's centre in PHYSICAL pixels — what the input handler is fed.
     *
     * <p>{@code UIWindow}'s default {@code uiScale} is 2, so logical coordinates handed straight to
     * {@code consumeMouseEvent} land at half the intended position and hit nothing. This is the
     * physical-vs-logical confusion {@code SplitViewDragTest} was written to guard, and it is just as easy
     * to walk into from a test as from a widget.</p>
     */
    private int[] centreOf(UIElement element) {
        var cache = element.getRuntimeCache();
        float scale = UI_SCALE;
        return new int[]{
                Math.round((cache.getX() + cache.getWidth() / 2f) * scale),
                Math.round((cache.getY() + cache.getHeight() / 2f) * scale)
        };
    }

    /** Physical pixels per logical pixel, matching {@code UIWindow}'s default. */
    private static final float UI_SCALE = 2f;

    private float widthOf(int paneIndex) {
        return split.pane(paneIndex).getRuntimeCache().getWidth();
    }
    ...
    public void removingAPaneRedistributesItsShare() {
        ...
        assertEquals("the survivors are back to filling the whole width",
                400f, totalPaneWidth() + split.divider().getRuntimeCache().getWidth(), 1f);
    }
     ...
     *
     * <p>"At least 150px" stays true at every window size; "at least 15%" does not, and the difference
     * only shows up on the window sizes nobody tested at.</p>
     */
    @Test
    ...
    public void threePanesWorkVerticallyToo() {
        ...
        assertEquals(3, split.paneCount());
        float firstBefore = split.pane(0).getRuntimeCache().getHeight();
        float middleBefore = split.pane(1).getRuntimeCache().getHeight();
        ...
        assertEquals("the first pane never moved",
                firstBefore, split.pane(0).getRuntimeCache().getHeight(), 1f);
        assertTrue("the middle one grew",
                split.pane(1).getRuntimeCache().getHeight() > middleBefore + 20f);
    }
}
