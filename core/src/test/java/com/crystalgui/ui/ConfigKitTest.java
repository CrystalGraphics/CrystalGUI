 ...
 */
public class ConfigKitTest extends UiTestBase {
    ...
    private static boolean isInsidePopup(UIElement e, UIElement stopAt) {
        for (UIElement p = e.getParent(); p != null && p != stopAt; p = p.getParent()) {
    ...
    public void everyControlLandsOnTheKitHeight() {
        ...
        for (ConfigDescriptor.Kind kind : registeredKinds()) {
            ...
            for (String tag : List.of("textfield", "dropdown", "slider")) {
                leaves.addAll(control.querySelectorAll(tag));
            }
    ...
    public void anEmptyArrayShowsItsPlaceholder() {
        ...
        assertFalse("an empty list must still show a row",
                array.querySelectorAll("." + ArrayControl.EMPTY_CLASS).isEmpty());
     ...
     * {@link com.crystalgui.ui.elements.config.control.MaskControl#writeToWidgets} rather than a
    ...
    public void aMaskSummarisesWhatIsActuallyChecked() {
        ...
        com.crystalgui.ui.elements.config.control.MaskControl mask =
                (com.crystalgui.ui.elements.config.control.MaskControl) row.control();
    ...
    public void theGalleryPageArrangementBuildsAndLaysOut() {
        ...
        for (UIElement row : panel.querySelectorAll("." + Configurator.ROW_CLASS)) {
            assertTrue("a row wider than its panel means the label column is not shrinking: "
                            + row.getRuntimeCache().getWidth() + " > " + panelWidth,
                    row.getRuntimeCache().getWidth() <= panelWidth + 0.5f);
        }
    }
    ...
    public void aNestedRowDoesNotOverflowThePanel() {
        ...
        float topRight = top.getRuntimeCache().getX() + top.getRuntimeCache().getWidth();
        float nestedRight = nested.getRuntimeCache().getX() + nested.getRuntimeCache().getWidth();
     ...
     *
     * <p>Unity's rule, from {@code docs/research/unity-inspector/07-full-window.png}: a row inside a
    ...
    public void theKitsColoursWinOverTheBaseWidgetRules() {
        ...
        UIElement field = text.control().querySelectorAll("textfield").get(0);
    ...
    public void hoveringAKitButtonOrDropdownIsNotSolidRed() {
        ...
        UIElement dropdown = select.control().querySelectorAll("dropdown").get(0);
        ...
        com.crystalgui.ui.elements.config.control.MaskControl maskControl =
                (com.crystalgui.ui.elements.config.control.MaskControl) mask.control();
    ...
    public void aCheckboxIsSquareAndSmallerThanTheRow() {
        ...
        UIElement mark = row.control().querySelectorAll(".__mark__").get(0);
        var c = mark.getRuntimeCache();
        assertEquals("a checkbox must be square", c.getWidth(), c.getHeight(), 0.5f);
        assertEquals("...at the checkbox token, not the kit height", 13f, c.getHeight(), 0.5f);
