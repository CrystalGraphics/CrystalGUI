 ...
 */
public class TaskbarPreviewTest extends UiTestBase {
    ...
    /**
     * A whole frame: tickers AND input.
     *
     * <p>{@code updateWithoutPainting} deliberately handles no input — no frame was presented, so hover
     * has nothing to be relative to — and the previews' clock is a ticker, which only advances inside
     * it. Both halves are needed, which is why this is not either helper on its own.</p>
     */
    private void frame() {
