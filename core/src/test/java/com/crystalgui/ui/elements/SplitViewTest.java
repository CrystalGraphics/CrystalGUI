...
public class SplitViewTest {
    ...
    public void rootRejectsPublicChildren() {
        SplitView sv = new SplitView();
        assertThrows(UnsupportedOperationException.class, () -> sv.addChild(new UIElement()));
    }
    ...
    public void panesAcceptChildren() {
        ...
        sv.first().addChild(a);
        sv.second().addChild(b);

        assertTrue(sv.first().hasChild(a));
        assertTrue(sv.second().hasChild(b));
    }
    ...
    public void paneContentRemainsRemovable() {
        ...
        sv.first().addChild(content);

        assertTrue(sv.first().removeChild(content));
        assertFalse(sv.first().hasChild(content));
    }
    ...
    public void clearingAPaneDoesNotDetachTheStructure() {
        ...
        sv.first().addChild(new UIElement());

        sv.first().clearAllChildren();

        assertSame("the pane itself must survive", first, sv.first());
        assertTrue(sv.first().getChildren().isEmpty());
    }
    ...
    public void firstAndSecondReplaceRatherThanAppend() {
        ...
        assertEquals(1, sv.first().getChildren().size());
        assertTrue(sv.first().hasChild(replacement));
        assertFalse(sv.first().hasChild(original));
    }
