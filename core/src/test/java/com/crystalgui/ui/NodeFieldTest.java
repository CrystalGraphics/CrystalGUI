 ...
 * <p>Everything asserted here is document and declaration behaviour, with no widget and no GL. That is
 ...
 * is to click it.</p>
 */
public class NodeFieldTest extends UiTestBase {
     ...
     *
     * <p>It is document state — it changes what the node does and must survive a reload — so this
     * project's rule puts it through an {@code Edit}. The first dropdown shipped writing the document
    ...
    public void aFieldChangeIsUndoable() {
        ...
        NodeData node = document.addNode(colourNode().create(0f, 0f));
        UndoStack undo = new UndoStack();

        undo.execute(SetNodeFieldEdit.of(document, node.id(), "Space", "World"));
        assertEquals("World", document.node(node.id()).properties().get("Space"));

        undo.undo();
        assertEquals("Object", document.node(node.id()).properties().get("Space"));
    ...
    public void consecutiveEditsToOneFieldMerge() {
        ...
        SetNodeFieldEdit first = SetNodeFieldEdit.of(document, node.id(), "Value", "vec4(0.5)");
        first.apply();
        SetNodeFieldEdit second = SetNodeFieldEdit.of(document, node.id(), "Value", "vec4(0.25)");
        second.apply();
        ...
        assertEquals("the whole run undoes at once",
                "vec4(1.0)", document.node(node.id()).properties().get("Value"));
    }
    ...
    public void differentFieldsDoNotMerge() {
        ...
        SetNodeFieldEdit value = SetNodeFieldEdit.of(document, node.id(), "Value", "vec4(0.5)");
        SetNodeFieldEdit space = SetNodeFieldEdit.of(document, node.id(), "Space", "World");
    ...
    public void aNoOpChangeIsRecognised() {
        ...
        assertFalse(SetNodeFieldEdit.of(document, node.id(), "Space", "Object").changesAnything());
        assertTrue(SetNodeFieldEdit.of(document, node.id(), "Space", "World").changesAnything());
    }
    ...
    public void anEditSurvivesItsNodeBeingGone() {
        GraphDocument document = new GraphDocument();
        NodeData node = document.addNode(colourNode().create(0f, 0f));
        SetNodeFieldEdit edit = SetNodeFieldEdit.of(document, node.id(), "Space", "World");
        document.removeNode(node.id());
