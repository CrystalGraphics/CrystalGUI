 ...
 * <p>{@code UIWindow.advanceFrame} ran {@code calculateStyle → tickAnimations → calculateLayout}, then
 ...
 */
public class BlackboardRowMeasureTest extends UiTestBase {
    ...
    /** Straight into the document, so no rename opens over the capsule and hides it. */
    private void declare(String name, String typeId) {
        document.addProperty(GraphProperty.of(name, typeId, "(0,0)"), document.propertyCount());
    }
    ...
    private float capsuleWidthOf(int row) {
        ...
        for (UIElement child : pill.getChildren()) {
            if (child.hasClass(PropertyPill.CAPSULE_CLASS)) {
                return child.getRuntimeCache().getWidth();
            }
        }
    ...
    public void aTickerDrivenRebuildMeasuresLikeAnyOther() {
        ...
        // A document change from INSIDE a ticker, which is the deferred refresh's real path: the panel
