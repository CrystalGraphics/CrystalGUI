 ...
 */
public class TextStylePropertiesCssTest extends UiDocumentTestBase {

    /** {@code UIDocument}'s constructor builds a {@code Input}, which asks the adapter how
     * many mouse buttons exist — a document cannot be constructed without one at all. */
    /**
     * Applies {@code css} to a root holding one {@link TextField} and returns the field's style.
     *
     * <p>{@code init()} is required, not incidental: it is what attaches the tree to the document, and
    ...
    private GeneralGroup styled(String css) {
        ...
        document.styleEngine().calculateStyle(0f);
    ...
    public void textOffsetInheritsToAnInternalLabel() {
        ...
        root.append(button);
        document.append(root);
        document.styleEngine().addStylesheet(StyleSheet.parse("#host { text-offset-y: 1px; }"));
        document.styleEngine().calculateStyle(0f);
