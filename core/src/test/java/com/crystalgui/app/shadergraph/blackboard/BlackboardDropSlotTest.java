...
public class BlackboardDropSlotTest extends UiDocumentTestBase {
    ...
    public void aRowMovesBothDownAndUp() {
        mountWith("Vector 2", "Vector 4");
        String vec2 = graphDocument.properties().get(0).id();
        ...
        frame();
        frame();
        assertEquals("it went down", 1, graphDocument.indexOfProperty(vec2));

        // And back up: above the first row.
        board.dropProperty(vec2, board.dropIndexAt(20f, screenYOf(board.pills().get(0), -1f)), "");
        frame();
        assertEquals("and back up again", 0, graphDocument.indexOfProperty(vec2));
    }
}
