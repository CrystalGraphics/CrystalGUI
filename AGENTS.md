...
| `cgui-library` | `CgUiLibraryScene` | The UI builder's Library: every placeable kind as a live card, every category open, Button selected. Writes `top`/`bottom` captures; `-Dcrystalgui.harness.library.width=140` docks it narrow, `-Dcrystalgui.harness.library.scroll=true` scrolls it forever and prints frame-time percentiles |
