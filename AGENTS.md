...
| `runtime/mc/shared/` | ✅ | **THE VARIANT SELECTOR** (J11.0) — Java 8, merged once and never relocated, for anything every loader variant must share without naming Minecraft. It holds `Variants` (the table a merged jar declares itself with, at `META-INF/<modid>/variants.json`), `VersionRange`, `VariantEntry` and `Bootstrap`: one jar carries every loader entry class and, above one Minecraft version per loader, several of each, so the loader constructs a **bootstrapper** that picks by loader and running version. The annotated bootstrapper is NOT here — it must be part of the mod for a scanner to find it, and this module reaches a dev run as a *library* on `additionalRuntimeClasspath` — so each loader module carries its own ~20 lines. Was empty until 2026-09-12. `LoaderProbe` (which loader this process is) and `CrashVariant` (the crash-report line that says which variant ran) lived here and are **CrystalGraphics' now**: CrystalGUI requires CrystalGraphics on every loader, so a second copy bought nothing. The hosts register under their own heading, `CrashVariant.label(NAME)`, and `checkSingleJar` forbids `com/crystalgraphics/` so a copy cannot creep back in as a split package. |
