...
| `plan_windowing.md` (repo root) | **live** | The window lifecycle: hide, close and destroy as three different things, the registry that owns retained windows, and the keybind-summoned strip that makes a hidden one findable. Researched against Win32, X11, Cocoa, the Page Lifecycle API and bfcache — close is a *request* everywhere, and a hidden thing must stop working |
