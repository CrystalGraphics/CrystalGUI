...
```bash
**1.20.x/1.21.x** — a node per (loader, Minecraft version): forge:1.13.2 | forge:1.14.3 | forge:1.14.4 | forge:1.15.2 | forge:1.16.5 | forge:1.17.1 | forge:1.18.2 | forge:1.19.2 | forge:1.19.3 | forge:1.19.4 | forge:1.20.1 | forge:1.20.2 | forge:1.20.4 | forge:1.20.6 | forge:1.21.1 | forge:1.21.3 | forge:1.21.4 | forge:1.21.5 | forge:1.21.6 | forge:1.21.8 | forge:1.21.10 | forge:1.21.11 | neoforge:1.20.2 | neoforge:1.20.3 | neoforge:1.20.4 | neoforge:1.20.6 | neoforge:1.21.1 | neoforge:1.21.3 | neoforge:1.21.4 | neoforge:1.21.5 | neoforge:1.21.6 | neoforge:1.21.8 | neoforge:1.21.10 | neoforge:1.21.11 | fabric:1.14.4 | fabric:1.15.2 | fabric:1.16.5 | fabric:1.17.1 | fabric:1.18.2 | fabric:1.19.2 | fabric:1.19.3 | fabric:1.19.4 | fabric:1.20.1 | fabric:1.20.2 | fabric:1.20.4 | fabric:1.20.6 | fabric:1.21.1 | fabric:1.21.3 | fabric:1.21.4 | fabric:1.21.5 | fabric:1.21.6 | fabric:1.21.8 | fabric:1.21.10 | fabric:1.21.11 — every release from 1.14.4 to 1.21.11 on Forge and Fabric (1.15.x and 1.16.x on one node each), 1.13.2 and 1.14.2-1.14.3 on Forge -- on Mojang names carried back from 1.14.4, since Mojang published none -- and from 1.20.2 on NeoForge, except Forge 1.21 (refused: Forge 51 has no HUD event), Fabric 1.16, 1.16.1 and 1.21.9 (refused: their only Fabric APIs lack a module the hosts use), Fabric 1.14-1.14.3 (the same, and 1.14.4's Fabric API fails on them) and the Forge versions never published (1.14, 1.14.1, 1.16, 1.17, 1.20.5, 1.21.2). Below 1.17 the nodes are built by Loom and Unimined rather than ModDevGradle, and Forge 1.13-1.16 instances run Java 8. Below 1.19.3 Minecraft ships no JOML, and those instances take CrystalGraphics' `crystalgraphics-joml` companion (`prismInstanceJoml`). NeoForge 1.21.2, 1.21.6, 1.21.7 and 1.21.9 run their only builds, betas; the NeoForge and Forge 1.21.10 nodes also claim 1.21.9. A node claims the versions it was booted on; its `variant.minecraft` says which.
...
./gradlew :runtime:mc:modern:<loader>:<version>:serverSmoke   # boots, asserts, stops. Needs -PcgAcceptEula
                                                  # once per run dir: the build detects Mojang's EULA
                                                  # and refuses to accept it for you.
./gradlew :runtime:mc:modern:<loader>:<version>:connectionProbe   # THE connection probe, driven. fabric
                                                  # needs -PcgNoLanguage: its dev client cannot load
                                                  # the language mod (the shipped jar is unaffected).
./gradlew checkAllTargets                         # every node compiled -- run it before committing
```
...
### Stub mode: every node without every Minecraft
...
| You | Then |
|---|---|
| change code in any node | nothing: the database holds the whole API |
...
> installed clients, not assumed. Not on ModLauncher 5 (Forge 1.15), which lists no resource inside a
> mod file — so discovery goes through `core.provider.Providers`, and that host names the copies itself.
...
| `runtime/mc/modern/` | ✅ | **In the build and running.** `common` holds the host — `CgUiScreen`, `HostModern`, `CgUiInput`, `CgUiHud`, `Connections`, `WorkspaceHostModern` and `LifecycleCrystalGUI`, which is **the one class a loader talks to**; `forge`/`neoforge`/`fabric` are registration only and forward into it. Every one of those is wiring: what opens, when it is raised, which arm paints and who may write are `core`'s — see the invariant, and `plan/crystalgui/platform-loader-cleanup.md`. **A Stonecutter tree since J11.1a** — one source tree, a node per Minecraft version, `:runtime:mc:modern:<branch>:<version>`, each node's pins in its own `versions/<version>/gradle.properties`; **read `runtime/mc/modern/README.md` before touching it**, and run `./gradlew checkAllTargets` before committing. CrystalGraphics has the same tree and owns its logic (`singlejar-logic`'s `ModernTree` and `ModernConventions`); every node here builds on CrystalGraphics' node of the same version. `common` is versioned too: **`neoforge` is MC 1.20.4** (NeoForge published no 20.1.x series) and compiles against `common:1.20.4`, built through NeoForm, while forge and fabric take `common:1.20.1`. All three boot a dedicated server and pass `./gradlew :runtime:mc:modern:<loader>:<version>:serverSmoke`; the desktop scene has been run on 1.20.1. Each loader also builds a **thin** jar — its own classes plus `common` relocated under `com.crystalgui.mc.<loader>.common` — which is what the root merge consumes; the fat per-loader jars still build on request and are on nobody's `assemble`. |
