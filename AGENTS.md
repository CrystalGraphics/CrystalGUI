...
| **THE UI RUNS WITH THE DEPTH FUNCTION AT `GL_ALWAYS`, AND A HOST RENDERER INHERITS IT** | The last and least visible half of handing GL to a foreign renderer, and it defeats a depth buffer that is present and correct. A UI paints in painter's order over whatever the world left behind, so `gui_curve.shader` sets `DepthTest ALWAYS` in its own `RenderState` deliberately — and a host that enables depth testing on top of that still has every fragment pass, which silently turns a depth-tested model back into submission order: the far face wins and a block item is drawn inside-out. **Nothing about it reads as a depth problem.** The target was complete throughout, `glIsEnabled(GL_DEPTH_TEST)` answered true, `GL_DEPTH_BITS` answered 24, and the write mask was on; the only wrong value in the whole state was `GL_DEPTH_FUNC` reading `0x0207`. Four other hypotheses were eliminated first — packed vs plain depth format, lazy mid-frame FBO creation, the harness context's own `needsDepthBuffer`, and a genuine bug where `glClear(GL_DEPTH_BUFFER_BIT)` is gated by the write mask and so cleared nothing. `NativeProfile.MODEL` therefore means *the engine establishes a depth-tested context*, `LEQUAL` and enabled, because no host should have to know what CrystalGUI left the function at |
...
| **VANILLA'S GUI GLINT CONSUMES AND THEN CLEARS DESTINATION ALPHA, AND BOTH HALVES ARE LOAD-BEARING** | `RenderItem.renderGlint` issues `OpenGlHelper.glBlendFunc(772, 1, 0, 0)` — **772 is `GL_DST_ALPHA`**, not `GL_DST_COLOR` — so it is `glBlendFuncSeparate(GL_DST_ALPHA, GL_ONE, GL_ZERO, GL_ZERO)`: it *reads* destination alpha as its source RGB factor and *writes* destination alpha to literal zero. The read is **why the glint comes out shaped like the item at all** — both icon branches end on `glBlendFunc(770, 771, 1, 0)`, i.e. `dst.a := src.a` under the alpha test, which exists to build exactly that mask. The write is **why the glint is ONE layer and not two** — `renderGlint` draws its quad twice, and pass 0 zeroing the alpha is the only thing that stops pass 1. Minecraft's own GUI target has real alpha throughout: `framebufferMc` is `GL_RGBA8` (`Framebuffer:113`), cleared to alpha 0 (`Minecraft:510`), and `fboEnable` defaults to true — so this is what a player actually sees, not a quirk of an alpha-less default framebuffer. `nativeContent` composites a real RGBA8 target **using** that alpha, so an enchanted stack erases its own coverage and disappears. **Every obvious repair is wrong, and each fails differently.** `glBlendEquationSeparate(GL_FUNC_ADD, GL_MAX)` makes alpha monotone and turns the multi-pass branch's deliberate 20×20 alpha *clear* (`RenderItem:481`, under its own `glColorMask(F,F,F,T)`) into a solid opaque square around every potion, spawn egg and dyed-leather item. `glColorMask(true, true, true, false)` around `renderEffect` protects the mask and **keeps pass 1 alive, doubling the glint** — which is a washed-out magenta sword and a leather chestplate with no shape left, and which reads as a colour bug rather than a pass count. Re-running `renderItemIntoGUI` under a colour mask to repair the damage is clobbered by that same branch restoring the mask to all-true at `:492` and painting the icon back over the finished glint. Deriving coverage from the depth buffer fails there too, because the `:481` quad rasterises untextured and alpha-test-free across the full 20×20 and therefore writes depth across the whole slot. **What works is to call vanilla verbatim and then RE-STATE the coverage from the icons themselves** — `getRenderPasses` + `getIcon` + `RenderItem.renderIcon`, all public, under `glColorMask(false, false, false, true)` and `glBlendFuncSeparate(GL_ZERO, GL_ONE, GL_ONE, GL_ZERO)`, with the alpha test **on** so several passes union instead of overwriting, and depth **off** so a `zLevel` vanilla has already restored cannot silently reject the entire repair |
| **`glAlphaFunc` is state vanilla only ever INHERITS, so hosting an item renderer means setting it yourself** | `RenderItem`'s 2D branch *enables* `GL_ALPHA_TEST` without ever setting the func, and `renderItemOverlayIntoGUI` disables blending before drawing the stack count through `FontRenderer`, which enables the test and nothing else. Every transparent glyph and icon texel is therefore discarded purely by whatever `glAlphaFunc` the process happens to be carrying — Minecraft's own ambient `GREATER 0.1`, set once at startup and re-asserted all over `EntityRenderer`, so it is simply always true in a vanilla frame. It is **not** always true here: no `CgGlSlot` models the alpha *func* (`ALPHA_TEST` tracks only the enable), the UI never writes one, and at the GL default of `(GL_ALWAYS, 0)` nothing is discarded at all — so the stack count draws a black rectangle through the item behind it, with blending off. Same class as `GL_RESCALE_NORMAL` and `setLightmapTextureCoords`: reproduce the host's **whole** preamble, not the subset that looks load-bearing |
| **A TILED NATIVE FILL PINS ITS GRID TO THE EDGE THAT MOVES, and only the SLOT knows which edge that is** | `FluidSlot.applyFill` narrows the content box to the filled portion before handing it over, so by the time a `NativeSurface` exists one of its edges moves with the tank and the other three do not — and a width and a height cannot say which. Pinned to a static edge, the cut tile lands on the *moving* one, so the fluid's surface shows a different slice of the sprite at every fill level and the seam walks as it fills: reported as *"it weirdly intersects when 2 tiles repeat"*. Pinned to the moving edge the surface is always a whole tile's edge, identical at every level, and the remainder falls against a border. Hence `NativeAnchor`, **defaulted** on `NativeSurface` so an item — every caller but one — neither answers it nor reads it. **The obvious minimal fix is a trap**: flipping the loop fixes `BOTTOM_UP` and breaks `TOP_DOWN` by the very same rule, because the direction is precisely what the loader cannot see. Within a tile the sprite then aligns to the *anchored* end on each axis — "repeat from the anchor, clip at the far end" — which is what keeps every join seamless; Tinkers' Construct pins its cut tile to `getMaxV()` instead, putting a discontinuity one tile above the tank floor, while its own horizontal axis disagrees with it and is never exercised (`SmelteryGui` always passes `endU = 16`) |
| **A DESCRIPTOR'S GRAMMAR IS CORE'S, and a loader that invents its own spelling changes what a serialised UI means per client** | A `NativeContent` descriptor is what a `NativeContentSlot` serialises, so it is the string a dedicated server authors, a description content-hashes, and a client of ANY version resolves — the one piece of native-content knowledge that must be identical everywhere. It started as three private prefix constants in `Mc1710Content`, which worked and was a drift waiting for its second loader. `NativeDescriptors` (core) now owns the three forms and the parsing rule that makes them unambiguous: **numerics are counted from the RIGHT**, because the id itself contains a colon — so `item:minecraft:5` keeps `5` as a *path* (an item id is exactly `ns:path`; consuming would leave one segment) while `fluid:minecraft:water:620:1000` and 1.7.10's bare `fluid:water:620:1000` parse under one grammar. Formatting is canonical (damage and count always written — a description is content-addressed, so two spellings of one stack would hash as two UIs); parsing is tolerant and answers **null, never a throw**, because a descriptor is wire data and the service contract for the unresolvable is already `EMPTY`. NBT is deliberately outside the grammar (unbounded text form; would re-hash on every change of a value nothing reads) — an NBT display stack is authored through a loader handle, and that is a boundary, not a gap. The harness stand-in parses the same grammar, so a drifting loader stops resolving in a scene anyone can run in seconds; `NativeDescriptorsTest` lives in **headlessTest** because a dedicated server authoring these strings with no CrystalGraphics present is the claim being pinned |
...
com.crystalgui.ui              UIElement, UIWindow, Ui, UITransform, EventListenerGroup,
  ...
  .elements                    Button, Checkbox, CheckboxGroup, Dialog, DialogManager, DragGhost,
    ...
    .slot                      NATIVE CONTENT — the one place the host draws inside a CrystalGUI element.
                               ...
                               given our coordinate space. NativeDescriptors is the DESCRIPTOR GRAMMAR
                               (slot:/item:/fluid:, numerics counted from the right) — core's, never a
                               loader's, pinned by a headlessTest contract test, so the same serialised
...
| `plan_native_content.md` (repo root) | **done but for in-game re-verification** | NativeContent as a cross-version contract — the Architectury property for `ItemSlot`/`FluidSlot`. The architecture already has the shape (elements, GL bracket, profiles, anchoring and tile arithmetic are all `core`; a loader contributes one service class); the plan closes the three remaining leaks: the descriptor grammar becomes core's (`NativeDescriptors`), descriptor-only authoring becomes the demonstrated path, and the cut-tile UV fractions join `NativeTileGrid`. Interaction (slot clicks) stays explicitly out of scope |
