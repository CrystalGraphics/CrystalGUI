...
| `language/` | ✅ | The language stack — everything with a native or an engine behind it. Depends on `core/`; **`core/` must never depend on it**, which is what keeps tree-sitter's `.so`s and ECJ's ~13MB off a dedicated server. `.grammar` (six tree-sitter grammars), `.engine` (band selection + the isolated loader), `.java` (the ECJ adapter), `.map` (the readable↔runtime boundary, on ASM), `.run` (the execution service). `.js`/`.resolve` are reserved. *(Was `syntax-treesitter/` until M4.)* |
