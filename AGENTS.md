...
| `language/` | ✅ | The language stack — everything with a native or an engine behind it. Depends on `core/`; **`core/` must never depend on it**, which is what keeps tree-sitter's `.so`s and ECJ's ~13MB off a dedicated server. Ships six tree-sitter grammars in `.grammar` and the engine-band loader in `.engine`; `.java`/`.js`/`.resolve` are reserved for the adapters. *(Was `syntax-treesitter/` until M4.)* |
