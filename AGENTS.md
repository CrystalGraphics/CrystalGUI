...
| **A window command is registered globally and resolves its subject from the CONTEXT — never `desktop.activeWindow()` at the call site** | Four routes reach the same operation (system menu via `Alt+Space`, via a title-bar right-click, via a taskbar right-click, plus the palette), and each arrives with a different `source` element. Resolving through `WindowFrame.WINDOW_FRAME` is what makes all four correct at once; a call site that asks the desktop for the active window is right for three of them and silently wrong for the fourth, which is the one a user reaches for precisely *because* the window is not in front |
