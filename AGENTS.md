...
| `language/` | ✅ | The language stack — everything with a native or an engine behind it. Depends on `core/`; **`core/` must never depend on it**, which is what keeps tree-sitter's `.so`s and ECJ's ~13MB off a dedicated server. `.grammar` (six tree-sitter grammars), `.engine` (band selection, the ONE shared loader per band — `EngineHost` — the language-neutral `Analysis` answer and the `AnalysedLanguageServices` attachment every engine extends), `.java` (everything Java: the ECJ adapters, `JavaLanguageServices`, and `ScriptHost`, the Java `ScriptRuntime`), `.js` (everything JavaScript: the Rhino adapters behind the band loader, `JsLanguageServices`, and `JsHost`, the JavaScript `ScriptRuntime` — see its `package-info` for which classes are on which side of the bridge, since the `Js`/`Rhino` prefixes no longer say), `.map` (the readable↔runtime boundary, on ASM), `.run` (the **engine-neutral** Run shell: `ScriptRuntime` SPI + `ScriptRuntimes` registry, commands, console, rail, sessions, and `ScriptPolicy` — which lives there because three of its four consumers are not JavaScript. `RunShellIsEngineNeutralTest` forbids it naming `.java`, `.js`, ECJ or Rhino). `.resolve` is reserved. *(Was `syntax-treesitter/` until M4.)* |
...
| **A child-side class may name only JDK types, `com.crystalgui.text.*` and the bridge — a shared utility is MOVED, never imported across** | The band loader is child-first over `com.crystalgui.language.*` and carries our own class files, so a child-side class importing `language.java` does not fail: it quietly defines a SECOND copy on the far side of the bridge, and two objects of "the same" type stop being assignable. `SimilarNames` was imported that way for a release — harmless, being stateless, and exactly the precedent not worth having. The rule is mechanical and so is the test for it: **a class that imports `org.mozilla.javascript` is child-side**, and `language/…/js/package-info.java` is the table, because the `Js`/`Rhino` prefixes stopped saying which is which the moment the fix catalog turned out to need the AST |
