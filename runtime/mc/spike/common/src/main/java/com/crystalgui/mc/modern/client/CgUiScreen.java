...
public final class CgUiScreen extends Screen {
    ...
    // `GuiGraphics` arrived in 1.20 and the same parameter is a `PoseStack` before it -- an
    // OVERRIDE, so no reflection or dual-spelling can absorb it and a compiled variant per era is
    // forced. The body never touches the parameter except to flush it, so the condition is a
    // signature rather than two bodies.
    @Override
    //? if >=1.20 {
    ...
    *///?}
        ...
        graphics.flush();
        //?} else {
        /*// SPIKE, NOT A PORT (E-A1 day 2). Pre-1.20 there is no GuiGraphics and so no flush() on
        // the parameter: the drain is the tesselator's own buffer source, which is a DIFFERENT
        // call and not an absent one. Left unimplemented deliberately -- this branch proves the
        // preprocessor can express the break, and a 1.19.4 client built from it would show exactly
        // the one-shade-darker compositing the comment above describes.
        *///?}
