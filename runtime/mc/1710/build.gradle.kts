...
/**
 * Bands 11 and 17, bundled when `-PcgBundleBands` asks for them.
 *
 * A 1.7.10 client on Java 17 -- which lwjgl3ify and GTNH make an ordinary configuration -- selects band
 * 17. A jar that does not carry it fetches it from where `download/locations.json` lists it.
 */
...
dependencies {
    ...
    // The bands, for bundling -- see bundleEngineBands.
    ...
    doLast {
        for (band in bundledBands) {
            ...
            directory.resolve("index.txt").writeText(
                "# Band $band engine jars, in classpath order. Written by :runtime:mc:1710:bundleEngineBands.\n"
                        + jars.joinToString("\n") + "\n")
        }
    }
}
...
//
//   ./gradlew :runtime:mc:1710:runClient -PcgNoDedup      never eliminate a GL call -- distinguishes "the state
//                                                shadow is lying" from a real rendering bug in one run
//   ./gradlew :runtime:mc:1710:runClient -PcgStateVerify  verify the shadow against the driver before eliding
...
tasks.named<JavaExec>(runTask) {
    ...
    if (providers.gradleProperty("cgBundledEngines").isPresent) {
        dependsOn(tasks.named("bundleEngineBands"))
        classpath(layout.buildDirectory.dir("engine-bundle"))
        // With -PcgBundleBands=none the client falls through to the DOWNLOAD, from the locations :language
        // bundles -- the only way to exercise that path on a host whose own band is the bundled one.
    } else {
    ...
    // UNATTENDED CAPTURE:  ./gradlew :runtime:mc:1710:runClient -PcgAutoTest
    //
...
val stageObfMods = tasks.register<Copy>("stageObfMods") {
    ...
    // CrystalGraphics is an INCLUDED BUILD, not a subproject, so `project(":CrystalGraphics:runtime:mc:1710")`
...
tasks.shadowJar {
    ...
    dependsOn(":runtime:mc:shared:jar")
}
...
// share: that class lives in `runtime/mc/modern/build-logic`, and this module applies the GTNH convention
