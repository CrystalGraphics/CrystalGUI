...
//  - The line below makes Gradle BUILD the lang source set for the run (`:runtime:mc:modern:forge:compileLangJava`
...
dependencies {
    ...
    // naming it. Its 1.20.x host half comes from :runtime:mc:modern:common's own lang source set.
    "langCompileOnly"(project(":language"))
    "langCompileOnly"(project(path = ":runtime:mc:modern:common", configuration = "commonLangOutput"))
    // AND ON THE RUNTIME CLASSPATH, like :core and :runtime:mc:modern:common above. compileOnly alone put it on
...
// Shared shadow JAR bundling: bundles :core and :runtime:mc:modern:common into shadowJar.
...
val langThinShadowJar = tasks.register<com.github.jengelman.gradle.plugins.shadow.tasks.ShadowJar>("langThinShadowJar") {
    group = "language jar"
    description = "This loader's language half plus :runtime:mc:modern:common's, relocated -- before remapping."
    ...
    val commonLangJar = project(":runtime:mc:modern:common").tasks.named<Jar>("langJar")
...
// nor :runtime:mc:modern:common:classes appears in `prepareClientRun --dry-run`.
//
...
//
//     Could not create task ':runtime:mc:modern:fabric:shadowJar'
//       > Task with name 'mergeDevServices' not found in project ':runtime:mc:modern:fabric'
//
 ...
 */
val cgBundledProjects = listOf(project(":core"), project(":runtime:mc:modern:common"), project(":language"))
...
tasks.withType<JavaExec>().matching { it.name == "runServer" }.configureEach {
    ...
    systemProperty("crystalgui.server.smoke.classdir",
            project(":runtime:mc:modern:common").extensions.getByType<SourceSetContainer>()["main"]
...
//
//   ./gradlew :runtime:mc:modern:forge:runClient -Dcrystalgui.layer.probe=true
...
//
//   ./gradlew :runtime:mc:modern:forge:runClient -Dcrystalgui.clientProbe=true [-PcgWorld="Some World"]
//
