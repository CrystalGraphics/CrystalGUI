...
//
// `project.name` is the VERSION here and the loader is the branch, so neither is ever spelled out
// below: `modernLoader` and `commonNode` say which is which. @see cgbuildlogic.ModernTree
useNodeCoordinates()
base { archivesName.set("crystalgui-$modernLoader-${project.name}") }

/** The common node of THIS node's Minecraft -- never another version's. */
val common: Project = project.commonNode

// Its tasks are read below at configuration time (the thin jar, the language thin jar, the bundled
// jars), which needs the project configured first. Declared rather than left to evaluation order,
 ...
 */
val cgBundledProjects = listOf(project(":core"), common, project(":language"))
...
//
// Downgrading the SHADOW jar rather than :core's own covers taffy and the common node in one pass and
...
tasks.withType<JavaExec>().matching { it.name == "runServer" }.configureEach {
    ...
    systemProperty("crystalgui.server.smoke.classdir",
            common.extensions.getByType<SourceSetContainer>()["main"].output.classesDirs.asPath)
...
//
//     ./gradlew :runtime:mc:modern:forge:1.20.1:connectionProbe
//
