 ...
 * vanilla host, relocated. Everything else — the engine, the language stack, Taffy, the engine
 * bands, tree-sitter — enters the merge <b>once, at the root</b>, so a copy here would be shipped
 * four times and would defeat the whole exercise. Nothing at runtime would notice, which is why this
...
abstract class CheckThinJar : DefaultTask() {
    ...
    fun check() {
        ...
        logger.lifecycle("[cgui] {}: {} classes, all under {}", file.name, classes, allowed)
    }
}
