...
- **Per-window zoom** (W15): a frame-level `Transform` scale — layout-free by design, so it is
...
| **W13b** ✅ | **Shipped 2026-08-23.** Alt-drag (capture-phase, modifier from `Desktop.moveModifier()`), fullscreen (`F11`, `window.fullscreen`), drag-to-edge snap over `SnapZones` | — | Snap is the same band arithmetic as `DockDropZones` and is a pure function over a rect, so the awkward cases are unit-testable. Fullscreen turned out to need no geometry of its own: **the strip is laid out, so hiding it re-flows the work area and a maximised window follows** — Windows' model falls out of the flex column. **One deviation, recorded below: the Alt-drag modifier is a setting, not a keymap binding** **Snap was revised 2026-08-24 after harness testing, and it needed all three of its parts changing.** (1) The zone was **quarters short**: only left/right halves and a top band existed, so dragging into a corner maximised. Corners are now the outer `CORNER_RATIO` of a side edge, which is KWin's published `ElectricBorderCornerRatio` and its default of **0.25** — the one number any of the references states outright. That inverts the old corner rule on purpose: a corner belonged to maximise only while there was nothing else for it to mean. (2) The trigger was **proximity where every reference uses CONTACT**. Windows fires when the cursor reaches the last row of pixels at the monitor edge, and the proof of the mechanism is that snap works on the far sides of a multi-monitor desktop and not on the interior edges where two monitors meet — those have no final row to arrive at. A 1px trigger feels deliberate precisely because the edge is a **wall**: the gesture is to shove the pointer into a stop. A twelve-pixel band fires while merely crossing the area, which is what shipped. The top is the exception and is derived rather than chosen: the pointer rides inside the caption for the whole drag and the window is clamped so the caption never leaves the work area, so a band shallower than the caption is unreachable for every grab but the shallowest — and because the pointer is inside the caption by construction, *within one caption of the top* and *the window has reached the top* are the same statement. (3) The **conversion was wrong**, which is what made it feel erratic rather than merely generous: `snapZoneAt` added the title bar's own origin to a pointer that was already in absolute layout coordinates, so both terms grew together as the window was dragged and the reported position ran at roughly double speed. A window near the right of the desktop armed RIGHT from the middle of the screen; one near the left could not reach LEFT at all. **`SnapZonesTest` could not see it** — that tests the arithmetic, and the fault was in the caller, so the covering tests now drive a real drag. AGENTS.md's own invariant row is what invited it, saying a drag callback's coordinates are "ALREADY LOCAL"; `screenToLocal` converts out of surface pixels and does **not** subtract the source's origin, and that row has been corrected. **A second pass the same day gave it motion and a surface.** The preview now MORPHS between shapes on a `WindowGeometryAnimation` — the driver a maximise uses — rather than appearing at its destination: it grows out of the window being dragged, re-aims when the zone changes, and contracts back into the window when the pointer leaves an edge. Growing out of the window is what makes it a *proposal* rather than a label, and it means both ends of every animation are a real place on screen, so nothing has to fade or pop. **A commit hides it at once instead** (`hideSnapPreviewNow`), because the window is about to take the very rect the preview occupies and contracting back to where the window used to be would play the gesture backwards beside the thing playing it forwards. Two things had to change for the timeline to be possible at all: the rect had been written at **IMPORTANT**, which sits permanently above the animation's own slot, and `showSnapPreview` is called **every frame** of a drag, so it has to no-op for an unchanged zone or the timeline is re-armed sixty times a second and eases toward a target it never reaches. The surface is now a **neutral translucent wash with a brighter edge**, with the transparency in the COLOURS rather than in `opacity`: `opacity` dims the border along with the fill, so a border lighter than its fill — the whole shape of the affordance — is unspellable, and it would put a work-area-sized element through the layer-FBO pass every frame of a drag. Grey rather than the accent, because an accent sheet over a window reads as a tint applied to that window rather than a region offered for it. **And the WINDOW animates in too**, which the first pass missed: `commitSnap` ended with `resizeTo` + `moveTo`, both instant, while the MAXIMIZE branch went through `maximize()` and had eased since W9 — so dragging to the top animated and dragging to a side teleported, one gesture behaving two ways. Invisible until the preview started morphing and then handed over to a window that simply appeared. `WindowFrame.snapTo` is the animated counterpart of `moveTo` + `resizeTo`, over the same `WindowGeometryAnimation` a maximise uses; the instant pair stays instant, because a session restoring twelve windows, a keyboard nudge keeping up with key repeat and a drag writing the pointer's position every frame must none of them animate. **A third pass fixed the preview's Z**, reported as "a zone that already holds a window will not take another". It always would — the snap lands correctly and a test driving the whole drag passes against the broken build, which is what made the report so precise: what was missing was the PREVIEW. `Desktop.raise` hands each window a `z-index` from a counter that only goes up, and the preview was hosted with none at all, so it sat at zero behind every window that had ever been clicked. Larger than the windows it hid under it read as working; put a window in the very half being previewed and it is entirely behind it. It now takes the slot the dragged window is vacating and the window moves up one — **above every other window and below the one in your hand**, because the obvious "put it on top" version draws the wash over the window being dragged and greys out the thing being acted on. **JOINT RESIZE followed, 2026-08-24** — Windows' own name for it, `JointResize`, shipped in Windows 10 build 10547 behind a setting that Windows 11 22H2 removed in favour of always-on. Windows 10 could pair TWO windows; 11 adapts the whole layout, which is what this does. **The group's state is the CUT, not the pair**: two windows sharing an edge are `n` and `1-n` of one axis, so `Desktop` keeps a fraction per axis and every member's rect is derived from it — which is what makes them unable to drift apart, and why a four-window layout needs no extra machinery. The vertical cut is shared by both rows, so dragging it between the top pair moves the bottom pair and the grid stays a grid; the centre corner is one gesture moving both cuts, which falls out of a corner handle reporting both axes. `SnapZones.Zone` gained an `(xSide, ySide)` algebra for this: a cell's divider is the edge facing the middle, so `handleDx == -xSide` answers "is this a divider" without a lookup table, and it turns `rectFor` into arithmetic. **An OUTER edge moves nothing else** — that is resizing one window, not repartitioning the screen. **Moving leaves the tile and resizing does not**, which is the distinction the whole feature rests on. The seam is a new `UIElement.onUserResize(dx, dy, width, height)` called last by `UIResizer`: `onLayoutChanged` cannot say WHICH edge was grabbed, and for anything tiled that is the only interesting part of the event. It carries the size because the drag has only just written it and the runtime cache is a frame behind — reading it back would leave the neighbour visibly trailing. |
| **W13c** ✅ | **Shipped 2026-08-24.** Show desktop (minimise-all with a memory that an activation drops), the modal-blocked pulse + the first `CgPlatform.sound()` consumer, keyboard Move/Size over `WindowKeyboardMove` | — | Move/Size is the only genuinely new interaction MODE here: modal, with no element to dispatch to, so it takes the rung the window switcher already occupies — intercepted in `consumeKeyboardEvent` ahead of dispatch, and **only the keys it acts on**; anything else ends the mode and is *not* eaten, so a mode nobody remembers entering cannot swallow the keyboard. The pulse is asked at the POINTER rather than of "is a modal open", because with per-window modality a press must blame the dialog in the window pressed |
...
2. **Default chords for `window.close` and `desktop.switchWindow`.** `Ctrl+W` and `Ctrl+Tab` are the
   ...
   - **The Alt-drag modifier is a SETTING, not a keymap binding. Resolved 2026-08-23 at W13b — a
     deviation from this document, recorded rather than quietly taken.** The requirement was
     "keymap-resolved, never a hardcoded Alt", and the reason behind it is sound and stands: Alt is
     contested territory here. But it cannot literally be a binding, and that is a fact about the keymap
     rather than a shortcut — a `KeyStroke` is a *key* plus modifiers, and `parse("Alt")` reads "Alt" as
     the key name and fails. There is no way to spell a modifier-only binding, and inventing one for a
     single gesture would mean teaching the resolver, the palette and the accelerator renderer about a
     stroke that can never be pressed. `Desktop.moveModifier()` is the substance without the letter: one
     place, changeable at runtime, and nothing in `WindowFrame` naming a modifier. GNOME keeps exactly
     this as a setting for exactly this reason (`mouse-button-modifier`).
