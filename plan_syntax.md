...
> ✅ **Both halves are built and proven** — the mapping boundary, and the live name environment M12 delivered.
>
...
>
> ✅ **Delivered at M12 (Phase 3).** This paragraph described `ReadableView` writing remapped classes to
> a directory and handing the path to the compiler, because `ASTParser.setEnvironment` takes file paths —
> correct anywhere bytes are obtainable, and not what a live MC host needs, where the bytes come from the
> launch classloader through the transformer chain.
>
> What shipped instead: `ScriptNameEnvironment`, an `INameEnvironment` with three tiers — the live
> runtime's remapped bytes, then the classpath delegate, then a reflective stub for a type the loader will
> hand over a `Class` for and no bytes. No writing and no staleness. **And the analyser takes the same
> environment as the compiler**, through `DomResolution` rather than `ASTParser`, which is the half that
> was nearly missed: a script compiled and ran perfectly while the editor showed red names, because only
> one of the two had been given it.
>
> Proven on a real client rather than only in tests: a script naming `net.minecraft.init.Blocks.stone`
...
| 2 | ~~`TSReader` chunked parse~~ | — | **Closed as decided against, not as unanswered.** The String path is adequate and the rope is handed to the worker rather than flattened on the frame, so the reason to try the chunked reader went away rather than being deferred. Left reading as an open question for three milestones, which is how a settled decision comes to look like owed work — the same rot §20's completeness contract is written against, arriving from the other direction |
