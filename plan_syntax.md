...
| **M12** ◐ | **Platform integration** — the one thing every milestone above deliberately stopped short of. **Phase 1 is designed in [`plan_m12.md`](plan_m12.md)** — read that for the 1.7.10 revival; this row is the whole milestone. Phase 1 is *getting the environment breathing*: `mc1710` back in the build and `CrystalEditor` open, painting and typeable in a real client. Two corrections that row cost: the platform services are **not** missing — CrystalGraphics owns all nine and always will, four of them more complete than the harness's — and `includeBuild("mc1710")` is the wrong restore, since `mc1710/settings.gradle` never existed and `git log -L` shows the configuration that ran was a plain `include(":mc1710")`. Phases 2+ then wire what the plan has been building against a stand-in: §15.5 A's **live name environment** reading post-transform bytes from `LaunchClassLoader` through the transformer chain, §15.5 C's **mapping data** (1.7.10 SRG↔MCP CSVs, `params.csv` for parameter names) with sourcing and licences settled, and the platform's `LanguageServices`/`ScriptHost` wiring. Then the same for `mc1201/` | M7, M11 | a script written in readable names compiles, runs and links **inside a real 1.7.10 client**, against MC classes and a mixin-added member; completion never shows `func_147439_a`; the same script runs unchanged in dev and prod |
...
| 11 | ~~Mapping data sourcing and licences~~ **DONE for 1.7.10** (2026-08-19) — MCP `stable_12` from MinecraftForge's FML repo, **fetched at runtime to the user's config directory and never redistributed**, which is the licensing position rather than a caching choice; version-addressed and pinned in the mod, absent-is-a-state, cache owned by the user. Recorded in `THIRD-PARTY.md` with the one open item stated (no upstream digests published, so a corrupt download is caught by the parse). **Mojang official / Parchment / Fabric tiny remain open and arrive with `mc1201`** | M6 | resolve, cache strategy decided, recorded in `THIRD-PARTY.md` |
